...
class RecoveryOrchestrator:
    ...
    def resume_simulation(self, target_end_year: int, config_hash: str, force_restart: bool = False) -> Optional[int]:
        ...
        if resume_year >= target_end_year:
            self.logger.info(f"Simulation already completed through year {resume_year}. Target: {target_end_year}")
            return None
    ...
    def _validate_resume_conditions(self, checkpoint: Dict[str, Any], current_config_hash: str) -> bool:
    ...
    def _validate_data_quality(self, checkpoint: Dict[str, Any]) -> bool:
        ...
        if 'error' in validation_data:
            self.logger.error(f"Data quality error in checkpoint: {validation_data['error']}")
            return False

        # Validate key metrics are reasonable
        total_compensation = validation_data.get('total_compensation', 0)
        if total_compensation < 0:
            self.logger.error(f"Invalid total compensation in checkpoint: {total_compensation}")
            return False

        baseline_count = validation_data.get('baseline_employee_count', 0)
        if baseline_count < 0:
            self.logger.error(f"Invalid baseline employee count in checkpoint: {baseline_count}")
            return False

        return True

    def calculate_config_hash(self, config_path: str = "config/simulation_config.yaml") -> str:
    ...
    def get_recovery_status(self, config_hash: str) -> Dict[str, Any]:
        ...
        resumable_year = self.checkpoint_manager.find_latest_resumable_checkpoint(config_hash)
        if resumable_year:
            ...
            if checkpoints:
                status['recommendations'].append("No resumable checkpoints due to configuration changes")
                status['recommendations'].append("Use --force-restart to start fresh")
            else:
                status['recommendations'].append("No checkpoints available - will start from beginning")

        return status

    def prepare_recovery_plan(self, start_year: int, end_year: int, config_hash: str) -> Dict[str, Any]:
        ...
        if resumable_year and resumable_year >= start_year:
            if resumable_year < end_year:
                ...
                plan['warnings'].append(f"Resuming from year {resumable_year}, skipping {years_saved} completed years")
            else:
                plan['recovery_mode'] = 'already_complete'
                plan['years_to_process'] = []
                plan['warnings'].append(f"Simulation already complete through year {resumable_year}")

        # Add warnings for configuration issues
        if resumable_year and resumable_year < start_year:
            plan['warnings'].append(f"Checkpoint available for year {resumable_year} but before requested start year {start_year}")
        ...
        if not checkpoints:
            ...
            for checkpoint in checkpoints:
                if not checkpoint.get('integrity_valid', True):
                    validation['warnings'].append(f"Checkpoint for year {checkpoint['year']} has no integrity validation")
