...
class CheckpointManager:
    ...
    def __init__(self, checkpoint_dir: str = ".navigator_checkpoints", db_path: str = "simulation.duckdb"):
        ...
        try:
            with duckdb.connect(self.db_path) as conn:
                ...
                tables = [
                    ...
                    'int_employee_contributions', 'int_baseline_workforce',
                    'int_employee_compensation_by_year', 'int_workforce_needs'
                ]
                ...
                try:
                    ...
                    event_types = conn.execute("""
                        ...
                        GROUP BY event_type
                    """, [year]).fetchall()
                    state['data_quality_metrics']['event_type_distribution'] = dict(event_types)
    ...
    def _capture_performance_metrics(self, year: int) -> Dict[str, Any]:
        ...
        metrics = {
            'checkpoint_creation_time': datetime.now().isoformat(),
            'year': year
        }
    ...
    def _save_atomic_checkpoint(self, year: int, checkpoint_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Checkpoint saved: {final_path} ({len(compressed_data)} bytes compressed)")
    ...
    def load_checkpoint(self, year: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not self._validate_checkpoint_integrity(checkpoint_data):
                raise CheckpointValidationError(f"Checkpoint integrity validation failed for year {year}")
    ...
    def _validate_checkpoint_integrity(self, checkpoint_data: Dict[str, Any]) -> bool:
        ...
        if stored_hash != calculated_hash:
            logger.error(f"Checkpoint integrity hash mismatch: stored={stored_hash[:8]}..., calculated={calculated_hash[:8]}...")
            return False
    ...
    def can_resume_from_year(self, year: int, current_config_hash: str) -> bool:
        ...
        if checkpoint_config_hash != current_config_hash:
            logger.warning(f"Configuration changed since checkpoint. Checkpoint: {checkpoint_config_hash}, Current: {current_config_hash}")
            return False

        # Validate database state matches checkpoint
        return self._validate_database_consistency(year, checkpoint)

    def _validate_database_consistency(self, year: int, checkpoint: Dict[str, Any]) -> bool:
                ...
                for table, expected_count in expected_counts.items():
                    ...
                    try:
                        ...
                        if actual_count != expected_count:
                            logger.error(f"Database inconsistency: {table} has {actual_count} rows, expected {expected_count}")
                            return False
