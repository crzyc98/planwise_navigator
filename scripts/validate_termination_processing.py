...
def validate_termination_processing(simulation_year: int = 2025) -> Dict[str, Any]:
    ...
    try:
        ...
        total_terminated = conn.execute(
            total_terminated_query, [simulation_year]
        ).fetchone()[0]
        ...
        validation_results["validation"]["overall_valid"] = (
            validation_results["validation"]["experienced_match"]
            and validation_results["validation"]["new_hire_match"]
            and validation_results["validation"]["total_match"]
        )
...
def print_validation_report(results: Dict[str, Any]) -> None:
    ...
    print(f"\n📊 EVENT COUNTS:")
    print(
        f"   • Experienced terminations: {results['events']['experienced_termination']:,}"
    )
    ...
    print(
        f"   • Experienced terminated: {results['snapshot']['experienced_termination']:,}"
    )
    ...
    if not validation["overall_valid"]:
        ...
        if validation["experienced_gap"] > 0:
            print(
                f"   • {validation['experienced_gap']} experienced termination events not applied"
            )
        ...
        if validation["new_hire_gap"] > 0:
            print(
                f"   • {validation['new_hire_gap']} new hire termination events not applied"
            )
            print(f"   • Check new hire termination processing logic")
        if validation["total_gap"] < 0:
            print(
                f"   • More terminated employees than events - possible data duplication"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate termination event processing"
    )
    parser.add_argument(
        "--year",
        type=int,
