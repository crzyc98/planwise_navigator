...
from orchestrator_mvp.core.state_management import (
    WorkforceStateManager,
    SimulationState,
    WorkforceMetrics,
    create_state_manager
)
from config.events import WorkforceEventFactory, DCPlanEventFactory, SimulationEvent
...
class BenchmarkScenario:
    ...
    def total_events(self) -> int:
        ...
        return self.workforce_size * len(self.simulation_years) * self.events_per_employee_per_year

    @property
    def complexity_score(self) -> float:
        """Calculate complexity score for scenario."""
        return (self.workforce_size * len(self.simulation_years) * self.events_per_employee_per_year) / 10000
...
class ComponentBenchmarkResult:
    ...
    def time_improvement_percent(self) -> float:
        ...
        if self.baseline_time_seconds == 0:
            return 0.0
        return ((self.baseline_time_seconds - self.optimized_time_seconds) / self.baseline_time_seconds) * 100
    ...
    def memory_improvement_percent(self) -> float:
        ...
        if self.memory_baseline_mb == 0:
            return 0.0
        return ((self.memory_baseline_mb - self.memory_optimized_mb) / self.memory_baseline_mb) * 100
    ...
    def throughput_improvement_percent(self) -> float:
        ...
        if self.operations_per_second_baseline == 0:
            return 0.0
        return ((self.operations_per_second_optimized - self.operations_per_second_baseline) / self.operations_per_second_baseline) * 100
...
class IntegrationBenchmarkResult:
    ...
    def total_time_improvement_percent(self) -> float:
        ...
        if self.total_baseline_time_seconds == 0:
            return 0.0
        return ((self.total_baseline_time_seconds - self.total_optimized_time_seconds) / self.total_baseline_time_seconds) * 100
    ...
    def coordination_overhead_reduction_percent(self) -> float:
        ...
        if self.coordination_overhead_baseline_seconds == 0:
            return 0.0
        return ((self.coordination_overhead_baseline_seconds - self.coordination_overhead_optimized_seconds) / self.coordination_overhead_baseline_seconds) * 100
...
class BenchmarkReport:
    ...
    integration_results: Dict[str, IntegrationBenchmarkResult] = field(default_factory=dict)
    ...
    def average_overhead_reduction_percent(self) -> float:
        ...
        if not self.integration_results:
            return 0.0
        reductions = [result.coordination_overhead_reduction_percent for result in self.integration_results.values()]
...
class CoordinationBenchmark:
    ...
    def __init__(self, verbose: bool = False):
        ...
        logger.info(f"Initialized CoordinationBenchmark with {len(self.scenarios)} test scenarios")
    ...
    def performance_context(self, operation_name: str):
        ...
        try:
            ...
            logger.debug(f"Completed '{operation_name}' in {duration:.3f}s, memory change: {memory_change:+.1f}MB")

    def generate_test_data(self, scenario: BenchmarkScenario) -> Tuple[List[SimulationEvent], WorkforceMetrics]:
        """
        ...
        """
        logger.info(f"Generating test data for scenario '{scenario.name}': {scenario.workforce_size:,} employees, {len(scenario.simulation_years)} years")
                ...
                for event_num in range(scenario.events_per_employee_per_year):
                    ...
                    try:
                        ...
                        if event_num == 0:
                            ...
                            # Merit event
                            new_compensation = Decimal(str(75000 + (employee_id % 50000) * 1.03))
                        ...
                        elif event_num == 2 and scenario.events_per_employee_per_year > 2:
                            ...
                            if event_num % 3 == 0:
                                # Promotion event
                                new_compensation = Decimal(str(75000 + (employee_id % 50000) * 1.05))
                                event = WorkforceEventFactory.create_promotion_event(
                                    employee_id=emp_id,
                                    scenario_id=scenario_id,
                                    plan_design_id=plan_design_id,
                                    effective_date=event_date,
                                    new_job_level=min(10, max(1, (employee_id % 8) + 2)),
                        ...
                        logger.warning(f"Failed to create event for {emp_id} (event {event_num}): {e}")
        ...
        workforce_metrics = WorkforceMetrics(
            ...
            employee_count_by_level={'L1': scenario.workforce_size // 2, 'L2': scenario.workforce_size // 3, 'L3': scenario.workforce_size // 6},
            demographics_summary={'age_range': '25-65', 'tenure_avg': 8.5}
        )

        logger.info(f"Generated {len(events):,} events and workforce metrics for scenario '{scenario.name}'")
        return events, workforce_metrics

    @with_error_handling("component_benchmark", enable_retry=False)
    def benchmark_cost_attribution(self, scenario: BenchmarkScenario, enable_optimization: bool = True) -> ComponentBenchmarkResult:
        """
        ...
        """
        logger.info(f"Benchmarking CrossYearCostAttributor - scenario: {scenario.name}, optimized: {enable_optimization}")
        ...
        cost_attributor = create_cost_attributor(
            scenario_id=f"benchmark_{scenario.name}",
            plan_design_id="benchmark_plan",
            allocation_strategy=AllocationStrategy.PRO_RATA_TEMPORAL if not enable_optimization else AllocationStrategy.HYBRID_TEMPORAL_WORKFORCE
        )

        # Create allocation context
        target_workforce_metrics = {year: workforce_metrics for year in scenario.simulation_years[1:]}
        allocation_context = create_allocation_context(
            ...
            allocation_strategy=AllocationStrategy.HYBRID_TEMPORAL_WORKFORCE if enable_optimization else AllocationStrategy.PRO_RATA_TEMPORAL
        )

        # Benchmark performance
        start_memory = psutil.Process().memory_info().rss / (1024**2)

        with self.performance_context(f"cost_attribution_{'optimized' if enable_optimization else 'baseline'}"):
            start_time = time.perf_counter()
            ...
            for i in range(min(100, len(events) // 100)):  # Sample of events
                ...
                cost_attributor.attribute_benefit_enrollment_costs(
                    benefit_events,
                    source_year=scenario.simulation_years[0],
                    target_year=scenario.simulation_years[1] if len(scenario.simulation_years) > 1 else scenario.simulation_years[0]
                )
        ...
        result = ComponentBenchmarkResult(
            ...
            operations_per_second_baseline=operations_per_second if not enable_optimization else 0.0,
            operations_per_second_optimized=operations_per_second if enable_optimization else 0.0
        )

        logger.info(f"CrossYearCostAttributor benchmark complete: {duration:.3f}s, {len(attribution_entries)} attributions, {operations_per_second:.1f} ops/sec")
        return result

    @with_error_handling("component_benchmark", enable_retry=False)
    def benchmark_cache_manager(self, scenario: BenchmarkScenario, enable_optimization: bool = True) -> ComponentBenchmarkResult:
        """
        ...
        Returns:
            Component benchmark results
        """
        logger.info(f"Benchmarking IntelligentCacheManager - scenario: {scenario.name}, optimized: {enable_optimization}")
        ...
        for i in range(scenario.workforce_size // 100):  # Sample operations
            ...
            cache_data = {
                ...
                'compensation_history': [75000 + (i % 10000) for _ in range(len(scenario.simulation_years))],
                'events': [f"event_{j}" for j in range(scenario.events_per_employee_per_year)]
            }
        ...
        with self.performance_context(f"cache_manager_{'optimized' if enable_optimization else 'baseline'}"):
            start_time = time.perf_counter()
            ...
            for cache_key, _ in cache_operations[:len(cache_operations)//2]:  # Access 50% of cached data
                cached_data = cache_manager.get(cache_key, CacheEntryType.WORKFORCE_STATE)
        ...
        logger.info(f"IntelligentCacheManager benchmark complete: {duration:.3f}s, {total_operations:.0f} operations, {operations_per_second:.1f} ops/sec, {hit_rate:.1%} hit rate")

        return result

    @with_error_handling("component_benchmark", enable_retry=False)
    def benchmark_coordination_optimizer(self, scenario: BenchmarkScenario, enable_optimization: bool = True) -> ComponentBenchmarkResult:
        """
        ...
        Returns:
            Component benchmark results
        """
        logger.info(f"Benchmarking CoordinationOptimizer - scenario: {scenario.name}, optimized: {enable_optimization}")
        ...
        with self.performance_context(f"coordination_optimizer_{'optimized' if enable_optimization else 'baseline'}"):
            start_time = time.perf_counter()
        ...
        actual_reduction = optimization_results.get('actual_overhead_reduction_percent', 0)
        logger.info(f"CoordinationOptimizer benchmark complete: {duration:.3f}s, {actual_reduction:.1f}% overhead reduction achieved")

        return result

    @with_error_handling("component_benchmark", enable_retry=False)
    def benchmark_resource_optimizer(self, scenario: BenchmarkScenario, enable_optimization: bool = True) -> ComponentBenchmarkResult:
        """
        ...
        Returns:
            Component benchmark results
        """
        logger.info(f"Benchmarking ResourceOptimizer - scenario: {scenario.name}, optimized: {enable_optimization}")
        ...
        with self.performance_context(f"resource_optimizer_{'optimized' if enable_optimization else 'baseline'}"):
            start_time = time.perf_counter()
        ...
        logger.info(f"ResourceOptimizer benchmark complete: {duration:.3f}s, {memory_savings:.1f}GB memory savings, {io_reduction:.1%} I/O reduction")

        return result

    def run_integration_benchmark(self, scenario: BenchmarkScenario) -> IntegrationBenchmarkResult:
        """
        ...
        for component_name, benchmark_func in component_benchmarks.items():
            try:
                ...
                logger.debug(f"Baseline {component_name}: {result.baseline_time_seconds:.3f}s")
            except Exception as e:
        ...
        for component_name, benchmark_func in component_benchmarks.items():
            try:
                ...
                logger.debug(f"Optimized {component_name}: {result.optimized_time_seconds:.3f}s")
            except Exception as e:
        ...
        integration_result.coordination_overhead_baseline_seconds = coordination_overhead_baseline
        integration_result.coordination_overhead_optimized_seconds = coordination_overhead_optimized

        logger.info(f"Integration benchmark complete for '{scenario.name}':")
        logger.info(f"  Total time: {total_baseline_time:.3f}s -> {total_optimized_time:.3f}s ({integration_result.total_time_improvement_percent:.1f}% improvement)")
        logger.info(f"  Coordination overhead: {coordination_overhead_baseline:.3f}s -> {coordination_overhead_optimized:.3f}s ({integration_result.coordination_overhead_reduction_percent:.1f}% reduction)")
        logger.info(f"  Target achieved: {integration_result.target_achieved}")

        return integration_result

    def run_all_scenarios(self, scenario_names: Optional[List[str]] = None) -> BenchmarkReport:
        """
        ...
        for scenario_name in scenario_names:
            ...
            logger.info(f"Starting benchmark for scenario '{scenario_name}': {scenario.description}")
            ...
            try:
                ...
                if integration_result.target_achieved:
                    logger.info(f"✅ Scenario '{scenario_name}' achieved 65% overhead reduction target")
                else:
                    logger.warning(f"⚠️ Scenario '{scenario_name}' did not achieve 65% overhead reduction target ({integration_result.coordination_overhead_reduction_percent:.1f}%)")

            except Exception as e:
                logger.error(f"Benchmark failed for scenario '{scenario_name}': {e}")
                # Create empty result to maintain report structure
                report.integration_results[scenario_name] = IntegrationBenchmarkResult(scenario_name=scenario_name)

        # Analyze overall results
        if report.integration_results:
            successful_scenarios = [r for r in report.integration_results.values() if r.target_achieved]
            report.overall_target_achieved = len(successful_scenarios) >= len(report.integration_results) * 0.8  # 80% success rate

            # Check for performance regression (simplified)
            avg_reduction = report.average_overhead_reduction_percent
            report.performance_regression_detected = avg_reduction < 40.0  # Threshold for regression

        logger.info(f"Benchmark testing complete. Overall performance grade: {report.performance_grade}")
        logger.info(f"Average overhead reduction: {report.average_overhead_reduction_percent:.1f}%")
        logger.info(f"Overall target achieved: {report.overall_target_achieved}")

        return report

    def generate_detailed_report(self, report: BenchmarkReport, output_file: Optional[Path] = None) -> str:
        """
        ...
        lines.append("-" * 40)
        lines.append(f"Test Date: {report.test_timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}")
        ...
        lines.append(f"Average Overhead Reduction: {report.average_overhead_reduction_percent:.1f}%")
        lines.append(f"Target Achieved (65% reduction): {'✅ YES' if report.overall_target_achieved else '❌ NO'}")
        lines.append(f"Performance Regression Detected: {'⚠️ YES' if report.performance_regression_detected else '✅ NO'}")
        lines.append("")
        ...
        lines.append(f"Available Memory: {memory_info.get('available_gb', 'Unknown')} GB")
        lines.append(f"CPU Cores: {report.system_info.get('cpu', {}).get('logical_cores', 'Unknown')}")
        ...
        for scenario_name, integration_result in report.integration_results.items():
            ...
            if scenario:
                ...
                lines.append(f"  Simulation Years: {len(scenario.simulation_years)} years ({scenario.simulation_years[0]}-{scenario.simulation_years[-1]})")
                lines.append(f"  Total Events: {scenario.total_events:,}")
                lines.append(f"  Complexity Score: {scenario.complexity_score:.1f}")

            lines.append(f"  Total Time: {integration_result.total_baseline_time_seconds:.3f}s → {integration_result.total_optimized_time_seconds:.3f}s")
            lines.append(f"  Time Improvement: {integration_result.total_time_improvement_percent:.1f}%")
            lines.append(f"  Coordination Overhead: {integration_result.coordination_overhead_baseline_seconds:.3f}s → {integration_result.coordination_overhead_optimized_seconds:.3f}s")
            lines.append(f"  Overhead Reduction: {integration_result.coordination_overhead_reduction_percent:.1f}%")
            lines.append(f"  Target Achieved: {'✅ YES' if integration_result.target_achieved else '❌ NO'}")

            # Component Results
            lines.append("  Component Performance:")
            for component_name, component_result in integration_result.component_results.items():
                lines.append(f"    {component_name}:")
                lines.append(f"      Time: {component_result.baseline_time_seconds:.3f}s → {component_result.optimized_time_seconds:.3f}s ({component_result.time_improvement_percent:+.1f}%)")
                lines.append(f"      Memory: {component_result.memory_baseline_mb:.1f}MB → {component_result.memory_optimized_mb:.1f}MB ({component_result.memory_improvement_percent:+.1f}%)")
                lines.append(f"      Throughput: {component_result.operations_per_second_baseline:.1f} → {component_result.operations_per_second_optimized:.1f} ops/sec ({component_result.throughput_improvement_percent:+.1f}%)")
        ...
        if report.integration_results:
            ...
            lines.append(f"Best Performing Scenario: {best_scenario} ({best_result.coordination_overhead_reduction_percent:.1f}% reduction)")
            lines.append(f"Worst Performing Scenario: {worst_scenario} ({worst_result.coordination_overhead_reduction_percent:.1f}% reduction)")
            ...
            for integration_result in report.integration_results.values():
                for component_name, component_result in integration_result.component_results.items():
                    ...
                    component_improvements[component_name].append(component_result.time_improvement_percent)

            lines.append("\nComponent Performance Summary:")
            for component_name, improvements in component_improvements.items():
                avg_improvement = statistics.mean(improvements) if improvements else 0
                lines.append(f"  {component_name}: {avg_improvement:+.1f}% average time improvement")
        ...
        lines.append("• CrossYearCostAttributor - Cost attribution across simulation years")
    ...
    def save_json_results(self, report: BenchmarkReport, output_file: Path) -> None:
        ...
        for scenario_name, integration_result in report.integration_results.items():
            ...
            for component_name, component_result in integration_result.component_results.items():
                json_data['integration_results'][scenario_name]['component_results'][component_name] = asdict(component_result)
...
Examples:
  ...
  python scripts/benchmark_multi_year_coordination.py --all-scenarios --generate-report --output-dir ./benchmark_results
    ...
    parser.add_argument('--scenario', choices=['small', 'medium', 'large'],
                       help='Run benchmark for specific scenario')
    parser.add_argument('--all-scenarios', action='store_true',
                       help='Run benchmarks for all scenarios')
    ...
    try:
        ...
        if args.generate_report:
            ...
            detailed_report = benchmark.generate_detailed_report(report, report_output_file)
            print("\n" + detailed_report)
        else:
            ...
            for scenario_name, result in report.integration_results.items():
                ...
                print(f"  Overhead Reduction: {result.coordination_overhead_reduction_percent:.1f}%")
                print(f"  Target Achieved: {'✅' if result.target_achieved else '❌'}")

        # Exit with appropriate code
        if report.overall_target_achieved:
            logger.info("✅ Benchmark completed successfully - performance targets achieved")
            sys.exit(0)
        else:
            logger.warning("⚠️ Benchmark completed - performance targets not fully achieved")
            sys.exit(1)
