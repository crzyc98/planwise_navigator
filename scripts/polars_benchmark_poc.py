...
class WorkforcePerformanceBenchmark:
    ...
    def benchmark_active_employee_filter(
        ...
        print(
            f"   Pandas: {pandas_result['execution_time_ms']}ms, {pandas_count:,} rows"
        )
        print(
            f"   Polars: {polars_result['execution_time_ms']}ms, {polars_count:,} rows"
        )
        print(f"   ✅ Results match: {pandas_count == polars_count}")

        return pandas_result, polars_result

    def benchmark_level_grouping(
        self, pandas_df: pd.DataFrame, polars_df: pl.DataFrame
    ):
        ...
        def pandas_grouping():
            return (
                pandas_df.groupby("level_id")["current_compensation"]
    ...
    def benchmark_compensation_analysis(
        self, pandas_df: pd.DataFrame, polars_df: pl.DataFrame
    ):
        ...
        def pandas_analysis():
            ...
            return (
                active_df.groupby(["level_id", "tenure_band"])["current_compensation"]
        ...
        def polars_analysis():
            return (
                polars_df.filter(pl.col("employment_status") == "active")
        ...
        print(
            f"   Pandas: {pandas_result['execution_time_ms']}ms, {pandas_result['result_rows']} groups"
        )
        print(
            f"   Polars: {polars_result['execution_time_ms']}ms, {polars_result['result_rows']} groups"
        )

        return pandas_result, polars_result

    def benchmark_complex_aggregation(
        self, pandas_df: pd.DataFrame, polars_df: pl.DataFrame
    ):
        ...
        def pandas_complex():
            return (
                pandas_df[pandas_df["employment_status"] == "active"]
    ...
    def generate_performance_report(self, benchmark_results: Dict[str, tuple]):
        ...
        for operation, (pandas_result, polars_result) in benchmark_results.items():
            ...
            print(
                f"{operation:<25} {pandas_time:>7.1f}ms {polars_time:>7.1f}ms {speedup:>7.1f}x"
            )
        ...
        print("-" * 60)
        print(
            f"{'TOTAL':<25} {total_pandas_time:>7.1f}ms {total_polars_time:>7.1f}ms {overall_speedup:>7.1f}x"
        )
        ...
        for mem in memory_results:
            ratio = (
                mem["pandas_memory"] / mem["polars_memory"]
                if mem["polars_memory"] > 0
                else 1
            )
            print(
                f"   • {mem['operation']}: Pandas {mem['pandas_memory']:.1f}MB, Polars {mem['polars_memory']:.1f}MB"
            )
    ...
    def run_benchmark(self):
        ...
        # Run benchmarks
        benchmark_results = {}
        benchmark_results["Active Filter"] = self.benchmark_active_employee_filter(
            self.pandas_df, self.polars_df
        )
        benchmark_results["Level Grouping"] = self.benchmark_level_grouping(
            self.pandas_df, self.polars_df
        )
        benchmark_results[
            "Compensation Analysis"
        ] = self.benchmark_compensation_analysis(self.pandas_df, self.polars_df)
        benchmark_results["Complex Aggregation"] = self.benchmark_complex_aggregation(
            self.pandas_df, self.polars_df
        )
...
def main():
    ...
    try:
        ...
        print(
            f"   Polars is {results['overall_speedup']:.1f}x faster on workforce operations"
        )
