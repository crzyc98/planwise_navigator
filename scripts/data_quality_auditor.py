...
class DataQualityAuditor:
    ...
    def log_issue(self, severity: str, table: str, column: str, issue_type: str,
                  description: str, count: int = 0, details: Dict = None):
    ...
    def get_table_list(self) -> List[str]:
        ...
        try:
            ...
            self.log_issue("ERROR", "DATABASE", "N/A", "CONNECTION",
    ...
    def get_table_schema(self, table_name: str) -> List[tuple]:
        ...
        try:
            ...
            self.log_issue("ERROR", table_name, "N/A", "SCHEMA",
        ...
        for col_info in column_info:
            ...
            if col_type in numeric_types:
                try:
                    ...
                    self.log_issue("ERROR", table_name, col_name, "NULL_CHECK",
        ...
        for col_info in column_info:
            ...
            if col_type == 'DOUBLE':
                try:
                    ...
                    self.log_issue("ERROR", table_name, col_name, "CASTING_CHECK",
                ...
                try:
                    ...
                    if table == 'int_employee_event_stream':
                        ...
                        for event_type, total, non_null, null_count in results:
                            if null_count > 0 and event_type not in ['hire', 'initial_state']:
                    ...
                    self.log_issue("ERROR", table, "N/A", "EVENT_CHECK",
        ...
        for col_info in column_info:
            ...
            if col_name in range_checks:
                ...
                try:
                    ...
                    self.log_issue("ERROR", table_name, col_name, "RANGE_CHECK",
    ...
    def print_report(self):
        ...
        if severity_counts:
            ...
            for severity, count in sorted(severity_counts.items()):
                emoji = {"CRITICAL": "🔴", "WARNING": "🟡", "ERROR": "❌"}.get(severity, "ℹ️")
        ...
        if self.issues:
            ...
            for i, issue in enumerate(self.issues[:20], 1):  # Show first 20 issues
                emoji = {"CRITICAL": "🔴", "WARNING": "🟡", "ERROR": "❌"}.get(issue['severity'], "ℹ️")
    ...
    def print_key_findings(self):
        ...
        casting_issues = [issue for issue in self.issues if issue['issue_type'] == 'CASTING_ISSUE']
        null_issues = [issue for issue in self.issues if issue['issue_type'] == 'NULL_VALUES']

        print("\n1. 🎯 TENURE DATA QUALITY ISSUES:")
        if tenure_issues:
            print("   • Found significant NULL values in tenure columns across multiple tables")
            print("   • Root cause: Event stream design intentionally sets tenure to NULL for non-hire events")
            print("   • Impact: Causes DOUBLE to INT64 casting failures in downstream models")
            print("   • Recommendation: Implement tenure calculation logic in event processing")

        print("\n2. 🔄 TYPE CASTING VULNERABILITIES:")
        if casting_issues:
            print(f"   • Found {len(casting_issues)} tables with potential casting issues")
            print("   • Primary issue: DOUBLE columns with NULL values being cast to BIGINT")
            print("   • Location: fct_workforce_snapshot.sql line 129")
            print("   • Recommendation: Add COALESCE or conditional logic before casting")

        print("\n3. 📊 NULL VALUE PATTERNS:")
        if null_issues:
            critical_nulls = [issue for issue in null_issues if issue['severity'] == 'CRITICAL']
            print(f"   • Found NULL values in {len(null_issues)} column/table combinations")
            if critical_nulls:
                print(f"   • {len(critical_nulls)} are critical (>50% NULL values)")
            print("   • Most problematic: current_tenure in int_employee_event_stream")
            print("   • Recommendation: Review event processing logic for tenure calculation")
