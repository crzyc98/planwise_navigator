...
def generate_birth_date():
    ...
    birth_date = datetime(2025, 1, 1) - timedelta(
        days=years_ago * 365 + _secure_rng.randint(0, 365)
    )
    ...
    if eligibility_date.day != 1:
        if eligibility_date.month == 12:
            ...
            eligibility_date = datetime(
                eligibility_date.year, eligibility_date.month + 1, 1
            )
...
def main():
    ...
    print(
        f"   Salary range: ${df['employee_gross_compensation'].min():,.0f} - ${df['employee_gross_compensation'].max():,.0f}"
    )
    print(
        f"   Deferral rate range: {df['employee_deferral_rate'].min():.1%} - {df['employee_deferral_rate'].max():.1%}"
    )
    print(
        f"   Non-participants (0% deferral): {(df['employee_deferral_rate'] == 0).sum():,}"
    )
    print(
        "   Edge cases included: 5 (recent hire, max comp, zero deferral, terminated, min comp)"
    )
    ...
    print("\nEdge cases:")
    print(
        df[df["employee_id"].str.startswith("EMP_2025_9999")][
            [
                "employee_id",
                "employee_hire_date",
