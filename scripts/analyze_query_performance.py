...
class QueryProfileAnalyzer:
        ...
        try:
            ...
            for query_name, query_sql in sample_queries.items():
                try:
                    ...
                    query_results.append(
                        {
                            ...
                            "performance_assessment": self._assess_query_performance(
                                execution_time, len(result)
                            ),
                        }
                    )
                    ...
                    query_results.append(
                        {"query_name": query_name, "error": str(e), "execution_time": 0}
                    )
    ...
    def _extract_query_info(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_usage_mb": profile.get("memory_usage", 0) / (1024 * 1024)
            if profile.get("memory_usage")
            else 0,
    ...
    def _analyze_performance_metrics(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_time = profile.get("total_time", 0)
        memory_usage_mb = (
            profile.get("memory_usage", 0) / (1024 * 1024)
            if profile.get("memory_usage")
            else 0
        )
                ...
                bottlenecks.append(
                    {
                        ...
                        "optimization_hints": self._get_operator_optimization_hints(
                            op_name
                        ),
                    }
                )
    ...
    def _generate_recommendations(
        ...
        for bottleneck in bottlenecks[:3]:  # Top 3 bottlenecks
            ...
            elif bottleneck["bottleneck_type"] in [
                "CPU_SORT",
                "CPU_JOIN",
                "CPU_AGGREGATE",
            ]:
    ...
    def _assess_e068e_targets(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        assessment = {
            ...
            if query_info["memory_usage_mb"] <= 4096
        ...
        # Overall assessment
        if (
            assessment["query_time_target"] == "PASS"
            and assessment["memory_efficiency"] == "GOOD"
        ):
    ...
    def _aggregate_metrics(self, analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "e068e_compliance_rate": ((total_queries - slow_queries) / total_queries)
            * 100,
        }
    ...
    def _identify_top_bottlenecks(
        ...
        for analysis in analyses:
            for bottleneck in analysis.get("bottlenecks", []):
                ...
                bottleneck_counts[bottleneck_type]["total_time"] += bottleneck[
                    "time_seconds"
                ]
                bottleneck_counts[bottleneck_type]["operators"].append(
                    bottleneck["operator"]
                )
        ...
        for analysis in analyses:
            for rec in analysis.get("optimization_recommendations", []):
                ...
                if "potential_improvement" in rec:
                    recommendation_counts[category]["potential_improvements"].append(
            ...
            prioritized.append(
                {
                    ...
                    "priority_score": high_priority_count * 3
                    + data["priorities"].count("MEDIUM") * 2
                    + data["priorities"].count("LOW"),
    ...
    def _generate_directory_report(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if analysis["top_bottlenecks"]:
            lines.extend(["TOP BOTTLENECKS", "-" * 15])
            for bottleneck in analysis["top_bottlenecks"][:5]:
                lines.append(
                    f"• {bottleneck['type']}: {bottleneck['frequency']} occurrences, {bottleneck['total_time']:.1f}s total"
                )

        # Priority recommendations
        if analysis["priority_recommendations"]:
            lines.extend(["", "PRIORITY RECOMMENDATIONS", "-" * 25])
            for rec in analysis["priority_recommendations"][:5]:
                lines.append(
                    f"• {rec['category']}: {rec['high_priority_instances']} high-priority instances"
                )
    ...
    def _generate_live_report(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        for query in successful_queries:
            lines.append(
                f"• {query['query_name']}: {query['execution_time']:.2f}s ({query['performance_assessment']})"
            )

        if failed_queries:
            lines.extend(["", "FAILED QUERIES", "-" * 14])
    ...
    def _generate_single_report(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        lines = [f"Profile: {analysis['profile_file']}", ""]
        ...
        if bottlenecks:
            lines.extend(["PERFORMANCE BOTTLENECKS", "-" * 22])
            for bottleneck in bottlenecks[:5]:
                lines.append(
                    f"• {bottleneck['operator']}: {bottleneck['time_seconds']:.2f}s ({bottleneck['percentage_of_total']:.1f}%)"
                )

        # Recommendations
        recommendations = analysis["optimization_recommendations"]
        if recommendations:
            lines.extend(["", "OPTIMIZATION RECOMMENDATIONS", "-" * 28])
...
def main():
    ...
    parser.add_argument(
        "profile_path", nargs="?", help="Path to query profile JSON file"
    )

    parser.add_argument(
        "--profile-dir", help="Directory containing query profile JSON files"
    )

    parser.add_argument(
        "--live-analysis", action="store_true", help="Perform live database analysis"
    )
