...
from scripts.benchmark_multi_year_coordination import (BenchmarkScenario,
...
def example_component_analysis():
    ...
    for component_name, benchmark_func in components:
        try:
            ...
            time_improvement = (
                optimized_result.time_improvement_percent
                if optimized_result.optimized_time_seconds > 0
                else 0
            )
            memory_improvement = (
                optimized_result.memory_improvement_percent
                if optimized_result.memory_optimized_mb > 0
                else 0
            )

            print(f"\n{component_name}:")
            print(
                f"  Time: {baseline_result.baseline_time_seconds:.3f}s → {optimized_result.optimized_time_seconds:.3f}s ({time_improvement:+.1f}%)"
            )
            print(
                f"  Memory: {baseline_result.memory_baseline_mb:.1f}MB → {optimized_result.memory_optimized_mb:.1f}MB ({memory_improvement:+.1f}%)"
            )
...
def example_comprehensive_analysis():
    ...
    for scenario_name, result in report.integration_results.items():
        ...
        print(
            f"{scenario_name.title()}: {result.coordination_overhead_reduction_percent:.1f}% reduction {status}"
        )
    ...
    if report.integration_results:
        ...
        print(
            f"Worst Performer: {worst_scenario} ({worst_result.coordination_overhead_reduction_percent:.1f}%)"
        )
...
def example_target_validation():
    ...
    results = {
        ...
        / len(report.integration_results)
        if report.integration_results
        else 0,
    }
    ...
    target_validations = {
        "overhead_reduction": results["average_reduction"]
        >= targets["minimum_overhead_reduction"],
        "grade_acceptable": results["performance_grade"] in ["A+", "A", "B+", "B"],
        "passing_rate_ok": results["passing_rate"]
        >= targets["required_scenarios_passing"],
    }
