...
def suggest_compensation_bands(
    comp_series: pd.Series, num_bands: int = 5
) -> pd.DataFrame:
    ...
    for i in range(num_bands):
        ...
        in_band = comp_series[
            (comp_series >= band_min)
            & (comp_series < band_max if i < num_bands - 1 else comp_series <= band_max)
        ]
        count = len(in_band)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze compensation ranges from census data"
    )
    parser.add_argument(
        "--census",
        type=Path,
    ...
    print(
        "- P5/P95 percentiles recommended to avoid outliers when defining custom ranges"
    )
