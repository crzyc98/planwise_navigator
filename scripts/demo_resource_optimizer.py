...
def demo_system_status():
    ...
    print(
        f"Memory: {status['memory']['available_gb']:.1f}GB available / {status['memory']['total_gb']:.1f}GB total"
    )
    print(
        f"Memory Usage: {status['memory']['used_percentage']:.1f}% (pressure: {status['memory']['pressure_level']})"
    )
    print(
        f"CPU: {status['cpu']['logical_cores']} cores, {status['cpu']['current_usage_percentage']:.1f}% current usage"
    )
    print(f"Disk: {status['disk']['free_space_gb']:.1f}GB free space")

    print(f"\nRecommendations:")
    print(
        f"- Suitable for large simulation: {'Yes' if status['recommendations']['suitable_for_large_simulation'] else 'No'}"
    )
    print(
        f"- Recommended max memory: {status['recommendations']['recommended_max_memory_gb']:.1f}GB"
    )
    print(
        f"- Streaming recommended: {'Yes' if status['recommendations']['streaming_recommended'] else 'No'}"
    )
...
def demo_memory_optimization(workforce_sizes: List[int], simulation_years: List[int]):
    ...
    for workforce_size in workforce_sizes:
        ...
        try:
            ...
            print(
                f"Recommended Chunk Size: {memory_result.recommended_chunk_size:,} employees"
            )
            print(
                f"Estimated Processing Time: {memory_result.estimated_processing_time_minutes:.1f} minutes"
            )
    ...
    for checkpoint_freq, persistence_level, description in scenarios:
        ...
        try:
            ...
            io_result = optimizer.optimize_io_operations(
                checkpoint_freq, persistence_level
            )

            print(f"Total I/O Reduction: {io_result.total_io_reduction_percentage:.1%}")
            print(
                f"Significant Improvement: {'Yes' if io_result.is_significant_improvement else 'No'}"
            )
            ...
            print(
                f"Checkpoint I/O Reduction: {checkpoint_opt['io_reduction_percentage']:.1%}"
            )
            ...
            print(
                f"Compression Savings: {compression_opt['estimated_savings_percentage']:.1%}"
            )
...
def demo_comprehensive_recommendations(
    workforce_size: int, simulation_years: List[int]
):
    ...
    print(
        f"- Simulation Years: {len(simulation_years)} years ({min(simulation_years)}-{max(simulation_years)})"
    )
    ...
    try:
        ...
        print(
            f"- Significant Improvement: {'Yes' if io_opt['significant_improvement'] else 'No'}"
        )
        ...
        for i, rec in enumerate(
            recommendations["overall_recommendation"]["key_recommendations"], 1
        ):
            print(f"{i}. {rec}")
    ...
    try:
        ...
        with optimizer.resource_monitor.monitor_context("demo_workload"):
            ...
            for i in range(5):
                ...
                print(
                    f"  Step {i+1}: Memory {metrics.memory_used_gb:.1f}GB ({metrics.memory_percentage:.1%}), CPU {metrics.cpu_percentage:.1%}"
                )
        ...
        if "error" not in summary:
            ...
            print(
                f"- Average Memory Usage: {summary['memory_usage']['average_percentage']:.1%}"
            )
            print(
                f"- Peak Memory Usage: {summary['memory_usage']['peak_percentage']:.1%}"
            )
            print(
                f"- Average CPU Usage: {summary['cpu_usage']['average_percentage']:.1%}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Demonstrate ResourceOptimizer capabilities"
    )
    parser.add_argument(
        "--workforce-size",
        type=int,
        default=25000,
        help="Workforce size for demonstrations",
    )
    parser.add_argument(
        "--years", type=int, default=5, help="Number of simulation years"
    )
    parser.add_argument(
        "--skip-monitoring", action="store_true", help="Skip resource monitoring demo"
    )
