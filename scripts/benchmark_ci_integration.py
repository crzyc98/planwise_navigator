...
class RegressionResult:
    ...
    def has_regression(self) -> bool:
        ...
        return (
            self.time_regression or self.throughput_regression or self.memory_regression
        )
...
class CIBenchmarkOrchestrator:
    ...
    def __init__(self, baseline_dir: Path = None, output_dir: Path = None):
        ...
        self.build_number = os.environ.get("BUILD_NUMBER") or os.environ.get(
            "GITHUB_RUN_NUMBER"
        )
    ...
    def run_regression_check(
        ...
        benchmark = EventGenerationBenchmark(
            output_dir=self.output_dir / "current", random_seed=12345
        )
        ...
        regression_analysis = self._analyze_regressions(
            current_results=current_results, baselines=baselines
        )
        ...
        if fail_on_regression and has_critical_regression:
            ...
            return {"status": "PASSED", "analysis": regression_analysis}

    def update_performance_baseline(
        self, scenarios: List[str] = None, modes: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        benchmark = EventGenerationBenchmark(
            output_dir=self.output_dir / "baseline_update", random_seed=12345
        )
    ...
    def run_performance_gate(
        ...
        benchmark = EventGenerationBenchmark(
            output_dir=self.output_dir / "performance_gate", random_seed=12345
        )
        ...
        if mode_results:
            ...
            if target_met:
                self.logger.info(
                    f"✅ Performance gate PASSED: {actual_time:.1f}s <= {target_time}s"
                )
            else:
                self.logger.error(
                    f"❌ Performance gate FAILED: {actual_time:.1f}s > {target_time}s"
                )

            return gate_result
        else:
            return {"status": "ERROR", "reason": "No results available for analysis"}
    ...
    def _save_baselines(self, baselines: List[PerformanceBaseline]):
        ...
        for baseline in baselines:
            baseline_file = (
                self.baseline_dir
                / f"{baseline.scenario_name}_{baseline.mode}_baseline.json"
            )
    ...
    def _create_baselines_from_results(
        ...
        for scenario_name, scenario_data in scenarios_tested.items():
            for mode, mode_results in scenario_data.get("results_by_mode", {}).items():
                baseline = PerformanceBaseline(
                    ...
                    baseline_events_per_second=mode_results.get(
    ...
    def _analyze_regressions(
        self, current_results: Dict[str, Any], baselines: Dict[str, PerformanceBaseline]
    ) -> Dict[str, Any]:
        ...
        for scenario_name, scenario_data in scenarios_tested.items():
            for mode, mode_results in scenario_data.get("results_by_mode", {}).items():
                ...
                time_change = (
                    (
                        (current_time - baseline.baseline_execution_time)
                ...
                throughput_change = (
                    (
                        (current_throughput - baseline.baseline_events_per_second)
                        / baseline.baseline_events_per_second
                        * 100
                    )
                    if baseline.baseline_events_per_second > 0
                    else 0
                )

                memory_change = (
                    (
                        (current_memory - baseline.baseline_memory_mb)
                ...
                regression_result = RegressionResult(
                    ...
                    > baseline.time_regression_threshold * 100,
                    throughput_regression=throughput_change
                    < -baseline.throughput_regression_threshold * 100,
                    memory_regression=memory_change
                    > baseline.memory_regression_threshold * 100,
                )
        ...
        critical_regressions = sum(
            1 for r in regressions if r.regression_severity == "critical"
        )
        major_regressions = sum(
            1 for r in regressions if r.regression_severity == "major"
        )
    ...
    def _generate_ci_report(self, regression_analysis: Dict[str, Any]):
        ...
        with open(summary_report, "w") as f:
            ...
            f.write(
                f"Build number: {regression_analysis.get('build_number', 'Unknown')}\n\n"
            )

            f.write(
                f"Total comparisons: {regression_analysis.get('total_comparisons', 0)}\n"
            )
            f.write(
                f"Total regressions: {regression_analysis.get('total_regressions', 0)}\n"
            )
            f.write(
                f"Critical regressions: {regression_analysis.get('critical_regressions', 0)}\n"
            )
            f.write(
                f"Major regressions: {regression_analysis.get('major_regressions', 0)}\n\n"
            )
            ...
            for regression in regressions:
                if regression.has_regression:
                    f.write(
                        f"REGRESSION: {regression.scenario_name} ({regression.mode})\n"
                    )
                    f.write(f"  Severity: {regression.regression_severity.upper()}\n")
                    f.write(f"  Time change: {regression.time_change_percent:+.1f}%\n")
                    f.write(
                        f"  Throughput change: {regression.throughput_change_percent:+.1f}%\n"
                    )
                    f.write(
                        f"  Memory change: {regression.memory_change_percent:+.1f}%\n\n"
                    )
...
Examples:
  ...
  python scripts/benchmark_ci_integration.py --mode regression-check --scenarios quick 1kx3 --modes polars
    ...
    parser.add_argument(
        "--modes", nargs="+", choices=["sql", "polars"], help="Generation modes to test"
    )
    ...
    try:
        if args.mode == "regression-check":
            ...
            if result["status"] == "FAILED":
                ...
                print(
                    f"Critical regressions: {analysis.get('critical_regressions', 0)}"
                )
                sys.exit(0)
            ...
            if result["status"] == "PASSED":
                print(
                    f"Target met: {result['actual_time']:.1f}s <= {result['target_time']}s"
                )
                print(
                    f"Margin: {result['margin']:.1f}s ({result['margin_percent']:.1f}%)"
                )
                sys.exit(0)
            else:
                print(
                    f"Target missed: {result.get('actual_time', 0):.1f}s > {result['target_time']}s"
                )
                sys.exit(1)
