...
from navigator_orchestrator import observability_session, ObservabilityManager
...
def run_dbt_command(command: List[str], obs: ObservabilityManager,
                   operation_name: str, **context) -> bool:
    """
    ...
    """
    with obs.track_operation(operation_name, command=" ".join(command), **context) as metrics:
        try:
            ...
            obs.log_info(f"dbt command completed successfully",
            ...
            obs.log_error(f"dbt command failed: {e}",
...
def validate_database_state(obs: ObservabilityManager, year: int) -> bool:
    ...
    with obs.track_operation("database_validation", year=year) as metrics:
        try:
            ...
            obs.log_data_quality_check(year, "workforce_snapshot_count", workforce_count, 50000)
            obs.log_data_quality_check(year, "yearly_events_count", events_count, 100000)
            ...
            if workforce_count == 0:
                obs.log_error(f"No workforce snapshot records found for year {year}", year=year)
                return False

            if events_count == 0:
                obs.log_warning(f"No yearly events found for year {year}", year=year)

            obs.log_info(f"Database validation passed for year {year}",
            ...
            obs.log_exception(f"Database validation failed for year {year}: {e}", year=year)
            return False
        finally:
            conn.close()
    ...
    with obs.track_operation("database_backup") as metrics:
        try:
            ...
            if backup_file.exists():
                ...
                obs.log_info("Database backup created successfully",
...
def run_simulation_year(year: int, config: Dict[str, Any], obs: ObservabilityManager) -> bool:
    """
    ...
    with obs.track_operation(f"simulate_year_{year}", year=year) as year_metrics:
        ...
        model_sequence = [
            ...
            ("workforce_needs", ["int_workforce_needs", "int_workforce_needs_by_level"]),
            ("event_generation", [
                "int_hiring_events", "int_termination_events", "int_new_hire_termination_events",
                "int_promotion_events", "int_merit_events", "int_enrollment_events"
            ]),
        ...
        for group_name, models in model_sequence:
            ...
            success = run_dbt_command(
                command, obs, f"run_{group_name}",
                year=year, group=group_name, models=models
            )

            if not success:
                obs.log_error(f"Failed to run {group_name} for year {year}",
                             year=year, group=group_name, models=models)
                return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enhanced Multi-Year Simulation Runner")
    ...
    with observability_session(run_id=args.run_id, log_level=args.log_level) as obs:
        try:
            ...
            with ExecutionMutex():
                ...
                for year in years:
                    obs.log_info(f"Processing year {year} ({years.index(year) + 1}/{len(years)})",
                                year=year,
                ...
                if failed_years:
                    ...
                    obs.log_error(f"Simulation completed with failures",
                                 successful_years=successful_years,
                                 failed_years=failed_years)
                else:
                    final_status = "success"
                    obs.log_info("All years completed successfully",
                                successful_years=successful_years)
