...
def generate_employee(employee_id: str, level: int, base_date: date) -> Dict[str, Any]:
    ...
    return {
        ...
        'level_id': level,
        'department': random.choice(['Engineering', 'Sales', 'Marketing', 'Operations', 'Finance', 'HR']),
        'location': random.choice(['Boston', 'NYC', 'Chicago', 'SF', 'Remote']),
        'age': age,
        'tenure_years': round(tenure, 1),
        'hire_date': hire_date.isoformat(),
        'current_compensation': salary,
        'performance_rating': random.choice([1, 2, 3, 3, 3, 4, 4, 4, 4, 5]),  # Weighted towards 3-4
        'active_flag': True
    }
...
# filename: tests/utils/generate_fake_census.py (continued)
    ...
    df.loc[terminated_indices, 'termination_date'] = pd.to_datetime(
        [fake.date_between(start_date='-2y', end_date='today') for _ in range(num_terminated)]
    ).astype(str)
...
def main():
    """CLI entry point."""
    parser = argparse.ArgumentParser(description='Generate synthetic employee census data')
    parser.add_argument('--employees', type=int, default=1000, help='Number of employees to generate')
    parser.add_argument('--output', type=str, default='data/synthetic_census.csv', help='Output file path')
