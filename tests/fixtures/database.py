...
@pytest.fixture
def events_explorer_db(tmp_path) -> Generator[Path, None, None]:
    """Create a deterministic, isolated database for event explorer tests."""
    db_path = tmp_path / "events_explorer.duckdb"
    conn = duckdb.connect(str(db_path))
    conn.execute(
        """
        CREATE TABLE fct_yearly_events (
          event_id VARCHAR, scenario_id VARCHAR, plan_design_id VARCHAR,
          employee_id VARCHAR, employee_ssn VARCHAR, event_type VARCHAR,
          simulation_year INTEGER, effective_date TIMESTAMP, event_details VARCHAR,
          compensation_amount DOUBLE, previous_compensation DOUBLE,
          employee_age BIGINT, employee_tenure DECIMAL(21, 2), level_id INTEGER,
          age_band VARCHAR, tenure_band VARCHAR, event_probability DOUBLE,
          event_category VARCHAR, event_sequence BIGINT,
          created_at TIMESTAMP WITH TIME ZONE, parameter_scenario_id VARCHAR,
          parameter_source VARCHAR, data_quality_flag VARCHAR,
