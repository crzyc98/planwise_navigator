...
class TestMVPOrchestratorPromotionIntegration:
    ...
    def _restore_tables(self):
        ...
        for table in tables:
            try:
                ...
                self.conn.execute(
                    f"CREATE TABLE {table} AS SELECT * FROM {table}_backup"
                )
    ...
    def test_full_pipeline_promotion_generation(self):
        ...
        self.conn.execute(
            "DELETE FROM fct_yearly_events WHERE event_type = 'promotion'"
        )

        # Run the orchestrator for one year
        orchestrator = MVPOrchestrator(
            self.conn, self.config, debug=True, use_mvp_models=True
        )
    ...
    def test_promotion_interaction_with_other_events(self):
        ...
        employees_with_multiple_events = employee_event_counts[
            employee_event_counts > 1
        ]

        assert (
            len(employees_with_multiple_events) > 0
        ), "No employees have multiple events"
        ...
        for emp_id in employees_with_multiple_events.index:
            ...
            promotion_dates = emp_events[emp_events["event_type"] == "promotion"][
                "effective_date"
            ].values
            termination_dates = emp_events[emp_events["event_type"] == "termination"][
                "effective_date"
            ].values

            for prom_date in promotion_dates:
                assert (
                    prom_date not in termination_dates
    ...
    def test_promotion_rates_by_level_integration(self):
        ...
        self.conn.execute(
            "DELETE FROM fct_yearly_events WHERE simulation_year = ?", [simulation_year]
        )
        ...
        for _, row in promotion_rates.iterrows():
            ...
            if level in expected_rates:
                ...
                assert (
                    expected * 0.7 <= actual_rate <= expected * 1.3
    ...
    def test_promotion_event_data_integrity(self):
        ...
        assert (
            promotions["simulation_year"].eq(simulation_year).all()
        ...
        for date_str in promotions["effective_date"]:
            ...
            assert (
                event_date.year == simulation_year
            ), f"Event date {date_str} outside simulation year"
    ...
    def test_promotion_performance_under_load(self):
        ...
        orchestrator = MVPOrchestrator(
            self.conn, self.config, debug=False, use_mvp_models=True
        )
        simulation_year = 2024
        ...
        assert (
            execution_time < 60
        ), f"Simulation took too long: {execution_time:.2f} seconds"
        ...
        if promotion_count > 0:
            ...
            ), f"Event generation too slow: {events_per_second:.2f} events/sec"
