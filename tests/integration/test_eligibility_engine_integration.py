...
from orchestrator_mvp.core.eligibility_engine import (
    EligibilityEngine,
    process_eligibility_for_year
)
from orchestrator_mvp.core.database_manager import get_connection, clear_database
...
class TestEligibilityEngineIntegration:
    ...
    def setup_method(self):
        ...
        try:
            ...
            test_employees = [
                ('EMP001', 'SSN-001', '2023-01-01', 'active', 30, 2.0, 2, 75000.0),  # 2+ years, eligible
                ('EMP002', 'SSN-002', '2024-06-01', 'active', 25, 0.5, 1, 60000.0),  # 6 months, not eligible
                ('EMP003', 'SSN-003', '2024-01-01', 'active', 35, 1.0, 3, 90000.0),  # 1 year, eligible
                ('EMP004', 'SSN-004', '2024-09-01', 'active', 28, 0.3, 2, 70000.0),  # 3 months, not eligible
                ('EMP005', 'SSN-005', '2022-01-01', 'active', 40, 3.0, 4, 120000.0), # 3+ years, eligible
            ]
    ...
    def test_eligibility_determination_with_database(self):
        ...
        config = {
            'eligibility': {
                'waiting_period_days': 365  # 1 year waiting period
            }
        }
    ...
    def test_multi_year_eligibility_progression(self):
        ...
        for year, events in events_by_year.items():
            ...
            assert len(overlap) == 0, f"Employee(s) {overlap} became eligible multiple times"
    ...
    def test_immediate_eligibility_configuration(self):
        ...
        config = {
            'eligibility': {
                'waiting_period_days': 0  # Immediate eligibility
            }
        }
        ...
        assert eligible_count == total_count, f"Only {eligible_count}/{total_count} eligible with immediate eligibility"
    ...
    def test_custom_waiting_period(self):
        ...
        assert eligible_count >= 3, f"Expected at least 3 eligible with 6-month waiting period, got {eligible_count}"
    ...
    def test_eligibility_engine_performance_integration(self):
        ...
        assert processing_time < 5.0, f"Processing 1000 employees took {processing_time:.2f} seconds, expected <5 seconds"
        ...
        print(f"✅ Performance test: Processed 1000 employees in {processing_time:.3f} seconds")
    ...
    def test_eligibility_edge_cases(self):
        ...
        try:
            ...
            # Edge cases:
            edge_cases = [
                ('EDGE001', 'SSN-E01', '2024-12-31', 'active', 25, 0.0, 1, 60000.0),  # Hired on Dec 31
                ('EDGE002', 'SSN-E02', '2024-01-01', 'active', 65, 1.0, 5, 200000.0), # Exactly 1 year
                ('EDGE003', 'SSN-E03', '2024-01-02', 'active', 21, 1.0, 1, 50000.0),  # 1 day short of 1 year
            ]
        ...
        assert edge001['is_eligible'] == False  # Hired Dec 31, 2024 - not eligible until 2026
