...
def test_multi_year_simulation_data_persistence(fresh_database):
    ...
    with fresh_database.get_connection() as conn:
        ...
        assert (
            events_count[1] == 2
        ), f"Expected 2 distinct years in events, found {events_count[1]}"
        assert (
            snapshot_count[0] == 4
        ), f"Expected 4 snapshots, found {snapshot_count[0]}"
        assert (
            snapshot_count[1] == 2
        ), f"Expected 2 distinct years in snapshots, found {snapshot_count[1]}"
...
def test_selective_data_clearing(fresh_database):
    ...
    with fresh_database.get_connection() as conn:
        ...
        assert (
            actual_event_years == expected_years
        ), f"Expected years {expected_years}, found {actual_event_years}"
        assert (
            actual_snapshot_years == expected_years
        ), f"Expected years {expected_years}, found {actual_snapshot_years}"
...
def test_enhanced_year_transition_validation(fresh_database):
    ...
    ), "Enhanced year transition validation should pass with comprehensive data"

    # Test validation with missing data
    validation_result_missing = validate_year_transition(2026, 2027)
    assert (
        validation_result_missing == False
    ), "Validation should fail when year 2026 data is missing"
...
def test_force_clear_vs_preserve_data_modes(fresh_database):
    ...
    with fresh_database.get_connection() as conn:
        ...
        assert (
            snapshots_count == 1
        ), "Snapshots should be preserved in preserve_data mode"
    ...
    with fresh_database.get_connection() as conn:
        ...
        events_count_after_clear = conn.execute(
            "SELECT COUNT(*) FROM fct_yearly_events WHERE simulation_year BETWEEN 2025 AND 2027"
        ).fetchone()[0]
        snapshots_count_after_clear = conn.execute(
            "SELECT COUNT(*) FROM fct_workforce_snapshot WHERE simulation_year BETWEEN 2025 AND 2027"
        ).fetchone()[0]

        assert (
            events_count_after_clear == 0
        ), "Events should be cleared in force_clear mode"
        assert (
            snapshots_count_after_clear == 0
        ), "Snapshots should be cleared in force_clear mode"
...
def test_data_handoff_validation(fresh_database):
    ...
    with fresh_database.get_connection() as conn:
        ...
        assert (
            result[0] == "previous_year_snapshot"
        ), "Should use previous year snapshot"
...
def test_performance_data_persistence(fresh_database):
    ...
    assert (
        operation_time < 10.0
    ), f"Data persistence operations took {operation_time:.2f}s, exceeding 10s target"
    assert (
        remaining_events[0] == 2000
    ), f"Expected 2000 remaining events, found {remaining_events[0]}"  # 2025 + 2027
    assert (
        remaining_events[1] == 2
    ), f"Expected 2 years remaining, found {remaining_events[1]}"
    assert (
        remaining_snapshots[0] == 2000
    ), f"Expected 2000 remaining snapshots, found {remaining_snapshots[0]}"
...
def test_circular_dependency_resolution(fresh_database):
    ...
    assert helper_result.get(
        "success", False
    ), f"Helper model build failed: {helper_result.get('error', 'Unknown error')}"
    ...
    with fresh_database.get_connection() as conn:
        ...
        assert (
            len(helper_data) == 3
        ), f"Expected 3 employees in helper model, found {len(helper_data)}"

        # Verify age and tenure incrementation and additional fields
        emp001_data = next(row for row in helper_data if row[0] == "EMP001")
        assert (
            emp001_data[1] == 45
        ), f"Expected age 45 for EMP001 (44+1), found {emp001_data[1]}"
        assert (
            emp001_data[2] == 6
        ), f"Expected tenure 6 for EMP001 (5+1), found {emp001_data[2]}"
        assert (
            emp001_data[3] == "45-54"
        ), f"Expected age_band '45-54', found '{emp001_data[3]}'"
        assert (
            emp001_data[4] == "5-9"
        ), f"Expected tenure_band '5-9', found '{emp001_data[4]}'"
        assert (
            emp001_data[5] == "previous_year_snapshot"
        ), f"Expected data_source 'previous_year_snapshot', found '{emp001_data[5]}'"
        assert (
            emp001_data[6] == "SSN001"
        ), f"Expected employee_ssn 'SSN001', found '{emp001_data[6]}'"
        assert (
            emp001_data[9] is None
        ), f"Expected termination_date to be None for active employee, found {emp001_data[9]}"

    # Test 3: Verify int_active_employees_by_year uses helper model correctly
    active_employees_result = run_dbt_model_with_vars(
        "int_active_employees_by_year", {"simulation_year": 2026}, full_refresh=True
    )

    assert active_employees_result.get(
        "success", False
    ), f"int_active_employees_by_year build failed: {active_employees_result.get('error', 'Unknown error')}"
    ...
    with fresh_database.get_connection() as conn:
        ...
        assert (
            len(active_employees_data) == 3
        ), f"Expected 3 active employees, found {len(active_employees_data)}"

        # Verify the dependency resolution method is marked
        for row in active_employees_data:
            assert (
                row[3] == "helper_model"
            ), f"Expected dependency resolution method 'helper_model', found '{row[3]}'"
...
def test_sequential_year_validation(fresh_database):
    ...
    assert (
        "sequential" in error_message.lower()
    ), f"Expected sequential execution error, got: {error_message}"
    assert (
        "2025" in error_message
    ), f"Expected error to mention year 2025, got: {error_message}"
    ...
    assert result_2025.get(
        "success", False
    ), f"Year 2025 execution failed: {result_2025.get('error', 'Unknown error')}"

    # Now year 2026 should work
    result_2026 = orchestrator_proper.execute_single_year(2026)
    assert result_2026.get(
        "success", False
    ), f"Year 2026 execution failed after 2025: {result_2026.get('error', 'Unknown error')}"
...
def test_helper_model_data_consistency(fresh_database):
    ...
    with fresh_database.get_connection() as conn:
        ...
        assert (
            emp001[3] == 77250
        ), f"EMP001 compensation should be preserved at 77250, got {emp001[3]}"
        assert (
            emp001[4] == "45-54"
        ), f"EMP001 age_band should be '45-54', got '{emp001[4]}'"
        assert (
            emp001[5] == "5-9"
        ), f"EMP001 tenure_band should be '5-9', got '{emp001[5]}'"
        assert (
            emp001[6] == True
        ), f"EMP001 data_quality_valid should be True, got {emp001[6]}"
        assert (
            emp001[7] is None
        ), f"EMP001 termination_date should be None for active employee, got {emp001[7]}"
        ...
        assert (
            emp002[3] == 66950
        ), f"EMP002 compensation should be preserved at 66950, got {emp002[3]}"
        assert (
            emp002[4] == "35-44"
        ), f"EMP002 age_band should be '35-44', got '{emp002[4]}'"
        assert (
            emp002[5] == "5-9 years"
        ), f"EMP002 tenure_band should be '5-9 years', got '{emp002[5]}'"
        assert (
            emp002[6] == True
        ), f"EMP002 data_quality_valid should be True, got {emp002[6]}"
        assert (
            emp002[7] is None
        ), f"EMP002 termination_date should be None for active employee, got {emp002[7]}"
