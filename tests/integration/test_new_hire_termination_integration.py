...
class TestNewHireTerminationIntegration:
    ...
    def test_end_to_end_simulation_with_fix(
        self, simulation_config, mock_workforce_data
    ):
        """Test complete simulation pipeline with termination date fix."""

        with patch(
            "orchestrator_mvp.core.workforce_calculations.get_current_workforce"
        ) as mock_workforce:
            mock_workforce.return_value = mock_workforce_data

            with patch(
                "orchestrator_mvp.core.event_emitter.generate_new_hire_termination_events"
            ) as mock_terminations:
                ...
                def side_effect_terminations(hire_events, rate, sim_year):
                    ...
                    return generate_new_hire_termination_events(
                        hire_events, rate, sim_year
                    )
        ...
        with patch(
            ...
            for year in range(2024, 2027):
                ...
                termination_events = generate_new_hire_termination_events(
                    hire_events, multi_year_config["new_hire_termination_rate"], year
                )

                termination_dates = [
                    event["effective_date"] for event in termination_events
                ]
    ...
    def test_database_integration_with_fix(self, mixed_hire_events):
        ...
        for event in termination_events:
            ...
            assert event["simulation_year"] == 2024, "simulation_year must match"

            # Validate date is realistic (not clustered)
            effective_date = event["effective_date"]
            assert effective_date >= date(2024, 1, 1), "Termination date too early"
            assert effective_date <= date(2024, 12, 31), "Termination date too late"
        ...
        assert (
            len(monthly_counts) >= 6
        ), "Should have terminations across multiple months"

        # December should not dominate
        dec_percentage = monthly_counts.get(12, 0) / len(termination_dates)
        assert (
            dec_percentage <= 0.35
        ), f"December dominance too high: {dec_percentage:.2%}"
    ...
    def test_workforce_calculation_compatibility(self, simulation_config):
        ...
        with patch(
            ...
            if requirements.get("new_hires", 0) > 0:
                ...
                termination_events = generate_new_hire_termination_events(
                    hire_events, simulation_config["new_hire_termination_rate"], 2024
                )

                # Should integrate seamlessly
                expected_terminations = int(
                    len(hire_events) * simulation_config["new_hire_termination_rate"]
                )
    ...
    def test_event_sequence_validation(self, mixed_hire_events):
        ...
        termination_events = generate_new_hire_termination_events(
            mixed_hire_events, 0.40, 2024  # Higher rate for more events to test
        )
        ...
        for term_event in termination_events:
            ...
            hire_event = next(
                h for h in mixed_hire_events if h["employee_id"] == employee_id
            )
    ...
    def test_edge_case_integration(self):
        ...
        termination_events = generate_new_hire_termination_events(
            december_hires, 1.0, 2024  # Force all to terminate for edge case testing
        )

        assert len(termination_events) == len(
            december_hires
        ...
        for event in termination_events:
            ...
            hire_event = next(
                h for h in december_hires if h["employee_id"] == event["employee_id"]
            )
            hire_date = hire_event["effective_date"]
