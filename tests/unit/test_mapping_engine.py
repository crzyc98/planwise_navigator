...
def test_string_case_upper():
    ...
    mappings = [_mapping("DEPT", "department", [{"transform_type": "string_case", "params": {"case": "upper"}}])]
    result = engine.apply(df, mappings)
...
def test_string_case_lower():
    engine = _engine()
    df = pd.DataFrame({"DEPT": ["Engineering", "Finance"]})
    mappings = [_mapping("DEPT", "department", [{"transform_type": "string_case", "params": {"case": "lower"}}])]
    result = engine.apply(df, mappings)
    assert list(result["department"]) == ["engineering", "finance"]
...
def test_string_case_title():
    engine = _engine()
    df = pd.DataFrame({"DEPT": ["ENGINEERING", "FINANCE"]})
    mappings = [_mapping("DEPT", "department", [{"transform_type": "string_case", "params": {"case": "title"}}])]
    result = engine.apply(df, mappings)
    assert list(result["department"]) == ["Engineering", "Finance"]
...
def test_date_parse_valid_format():
    ...
    mappings = [_mapping("HIRE_DATE", "hire_date", [{"transform_type": "date_parse", "params": {"format": "%m/%d/%Y"}}])]
    result = engine.apply(df, mappings)
...
def test_null_replace_fills_nulls():
    ...
    mappings = [_mapping("SALARY", "salary", [{"transform_type": "null_replace", "params": {"value": 0}}])]
    result = engine.apply(df, mappings)
...
def test_null_drop_removes_null_rows():
    engine = _engine()
    df = pd.DataFrame({"SALARY": [50000.0, None, 70000.0]})
    mappings = [_mapping("SALARY", "salary", [{"transform_type": "null_drop", "params": {}}])]
    result = engine.apply(df, mappings)
    assert len(result) == 2
...
def test_calculated_field_string_concat():
    ...
    mappings = [_mapping("FIRST", "full_name", [{"transform_type": "calculated_field", "params": {"expression": "FIRST + ' ' + LAST"}}])]
    result = engine.apply(df, mappings)
...
def test_calculated_field_arithmetic():
    ...
    mappings = [_mapping("BASE", "total", [{"transform_type": "calculated_field", "params": {"expression": "BASE + BONUS"}}])]
    result = engine.apply(df, mappings)
...
def test_chain_case_then_null_replace():
    ...
    mappings = [_mapping("DEPT", "department", [
...
def test_calculated_field_rejects_dangerous_expressions(dangerous_expr: str):
    ...
    mappings = [_mapping("A", "b", [{"transform_type": "calculated_field", "params": {"expression": dangerous_expr}}])]
