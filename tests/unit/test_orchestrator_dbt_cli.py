...
from run_multi_year import (PerformanceMonitor, error_context,
                            load_and_validate_config, parse_arguments,
...
class TestConfigurationValidation(unittest.TestCase):
    ...
    def test_validate_configuration_invalid_rates(self):
        ...
        invalid_config = {
            ...
            "workforce": {"total_termination_rate": -0.1},  # Negative
            "random_seed": 42,
        }
    ...
    def test_load_and_validate_config_from_file(self):
        ...
        test_config = {
            ...
            "workforce": {"total_termination_rate": 0.15},
            "random_seed": 123,
        }
...
class TestAsyncFunctions(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_foundation_benchmark_success(
        self, mock_load_config, mock_create_orchestrator
    ):
        ...
        with patch(
            "orchestrator_dbt.run_multi_year.OptimizationLevel"
        ) as mock_opt_level:
    ...
    @patch("orchestrator_dbt.run_multi_year.create_multi_year_orchestrator")
    @patch("orchestrator_dbt.run_multi_year.load_and_validate_config")
    async def test_run_foundation_benchmark_failure(
        self, mock_load_config, mock_create_orchestrator
    ):
...
class TestConfigurationCompatibility(unittest.TestCase):
    ...
    @patch("orchestrator_dbt.run_multi_year.load_and_validate_config")
    def test_configuration_compatibility_success(
        self, mock_load_config, mock_mvp_orchestrator
    ):
    ...
    @patch("orchestrator_dbt.run_multi_year.MultiYearSimulationOrchestrator")
    @patch("orchestrator_dbt.run_multi_year.load_and_validate_config")
    def test_configuration_compatibility_legacy_failure(
        self, mock_load_config, mock_mvp_orchestrator
    ):
    ...
    def test_configuration_compatibility_no_mvp(self, mock_load_config):
        ...
        self.assertTrue(result["new_system_compatible"])
        self.assertFalse(
            result["legacy_compatible"]
        )  # Should be false when MVP not available
    ...
    try:
        ...
        pytest.main(
            [
                __file__,
                "-v",
                "--cov=orchestrator_dbt.run_multi_year",
