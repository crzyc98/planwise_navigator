...
class TestRunYearSimulation:
    ...
    def test_successful_single_year_simulation_2025(
        ...
        expected_calls = [
            ...
            (
                ...
                False,
                "int_workforce_previous_year for year 2025",
            ),
            # fct_yearly_events
            (
                ...
                {"simulation_year": 2025},
                False,
                "fct_yearly_events for year 2025",
            ),
            ...
            (
                ...
                {"simulation_year": 2025},
                False,
                "fct_workforce_snapshot for year 2025",
            ),
        ]

        assert mock_execute_dbt.call_count == 3
        for i, (command, vars_dict, full_refresh, description) in enumerate(
            expected_calls
        ):
    ...
    def test_successful_multi_year_simulation_2026(
        ...
        mock_context.log.info.assert_any_call(
            "Previous year validation passed: 50 events, 1000 active employees from 2025"
        )
    ...
    def test_missing_workforce_snapshot_recovery(
        ...
        assert (
            "Previous year 2025 has events (50) but no workforce snapshot"
            in warning_call
        )
        ...
        mock_context.log.info.assert_any_call(
            "Successfully recovered workforce snapshot for year 2025: 1000 active employees"
        )
    ...
    def test_configuration_parameter_passing(
        ...
        mock_event_models.assert_called_once_with(
            mock_context, 2027, mock_context.op_config
        )
...
class TestRunYearSimulationIntegration:
    ...
    def test_complete_simulation_workflow(
        ...
        integration_context.log.info.assert_any_call(
            "Starting simulation for year 2025"
        )
        integration_context.log.info.assert_any_call(
            "Running int_workforce_previous_year for year 2025"
        )
        integration_context.log.info.assert_any_call(
            "Running event models for year 2025"
        )
        integration_context.log.info.assert_any_call(
            "Running fct_yearly_events for year 2025"
        )
        integration_context.log.info.assert_any_call(
            "Running fct_workforce_snapshot for year 2025"
        )
        integration_context.log.info.assert_any_call(
            "Year 2025 simulation completed successfully"
        )
