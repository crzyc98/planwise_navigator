...
class TestEligibilityEngine:
    ...
    def test_initialization_invalid_waiting_period(self):
        ...
        with pytest.raises(ValueError, match="waiting_period_days must be non-negative integer"):
    ...
    def test_generate_eligibility_events_no_newly_eligible(self, mock_get_connection):
        ...
        empty_data = pd.DataFrame(columns=[
            'employee_id', 'employee_hire_date', 'days_since_hire', 'was_previously_eligible'
        ])
...
class TestEligibilityEnginePerformance:
    ...
    def test_performance_with_large_dataset(self, mock_get_connection):
        ...
        assert processing_time < 5.0, f"Processing took {processing_time:.2f} seconds, expected <5 seconds"
    ...
    def test_memory_efficiency(self, mock_get_connection):
        ...
        assert total_size < 1024 * 1024, f"Event storage uses {total_size} bytes, expected <1MB"
