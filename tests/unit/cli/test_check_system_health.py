...
class TestCheckSystemHealth:
    ...
    def _make_wrapper(
        self, healthy: bool, issues: list[str] | None = None
    ) -> MagicMock:
    ...
    def test_unhealthy_system_raises_exit(self, mock_show_error, mock_console):
        ...
        wrapper = self._make_wrapper(healthy=False, issues=["Database is locked"])
    ...
    def test_unhealthy_system_shows_all_issues(self, mock_show_error, mock_console):
        ...
        printed_texts = [call[0][0] for call in mock_console.print.call_args_list]
        for issue in issues:
            assert any(
                issue in text for text in printed_texts
    ...
    def test_healthy_system_does_not_call_show_error(self):
        ...
        with patch(
            "planalign_cli.commands.simulate.show_error_message"
        ) as mock_show_error:
            _check_system_health(wrapper)
