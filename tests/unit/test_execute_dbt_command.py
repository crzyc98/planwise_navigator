...
class TestExecuteDbtCommand:
    ...
    def test_basic_dbt_run_command(self, mock_context):
        ...
        mock_context.log.info.assert_any_call(
            "Executing: dbt run --select test_model --vars {simulation_year: 2025}"
        )
        mock_context.log.info.assert_any_call("Description: test model execution")
        mock_context.log.info.assert_any_call(
            "Successfully completed: dbt run --select test_model"
        )
    ...
    def test_various_variable_types(self, mock_context):
        ...
        expected_vars = (
            "{string_var: test_value, int_var: 42, float_var: 3.14, bool_var: True}"
        )
...
class TestExecuteDbtCommandStreaming:
    ...
    def test_streaming_basic_execution(self, mock_context):
        ...
        results = list(
            execute_dbt_command_streaming(
                mock_context, ["build"], {}, False, "streaming test"
            )
        )
        ...
        mock_context.log.info.assert_any_call(
            "Successfully completed (streaming): dbt build"
        )
...
class TestExecuteDbtCommandIntegration:
    ...
    def test_realistic_simulation_workflow(self, integration_context):
        ...
        commands = [
            ...
            (
                ["run", "--select", "int_termination_events"],
        ...
        for command, vars_dict in commands:
            execute_dbt_command(
                integration_context, command, vars_dict, False, f"workflow {command[2]}"
            )
