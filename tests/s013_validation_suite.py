...
class S013ValidationFramework:
        ...
        try:
            ...
            with patch('orchestrator.simulator_pipeline.duckdb.connect') as mock_connect:
                with patch('orchestrator.simulator_pipeline.execute_dbt_command'):
                    with patch('orchestrator.simulator_pipeline._run_dbt_event_models_for_year_internal'):
                        with patch('orchestrator.simulator_pipeline.validate_year_results') as mock_validate:
                            ...
                            required_fields = [
                                'year', 'success', 'active_employees', 'total_terminations',
                                'experienced_terminations', 'new_hire_terminations',
                                'total_hires', 'growth_rate', 'validation_passed'
                            ]
    ...
    def _validate_multi_year_orchestration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with patch('orchestrator.simulator_pipeline.clean_duckdb_data') as mock_clean:
                with patch('orchestrator.simulator_pipeline._execute_single_year_with_recovery') as mock_execute:

                    # Setup successful execution
                    mock_execute.side_effect = [
                        YearResult(year=2025, success=True, active_employees=1030,
                                 total_terminations=120, experienced_terminations=100,
                                 new_hire_terminations=20, total_hires=150,
                                 growth_rate=0.03, validation_passed=True),
                        YearResult(year=2026, success=True, active_employees=1061,
                                 total_terminations=125, experienced_terminations=105,
                                 new_hire_terminations=20, total_hires=156,
                                 growth_rate=0.03, validation_passed=True)
                    ]
                    ...
                    orchestration_checks = {
                        ...
                        "correct_year_sequence": [r.year for r in results] == [2025, 2026],
    ...
    def _validate_error_handling(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                baseline_test = {"success": False, "error": "Should have raised exception"}
            except Exception as e:
                baseline_test = {
    ...
    def _validate_mathematical_accuracy(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for case in test_cases:
                ...
                total_hires_needed = math.ceil((experienced_terms + growth_amount) / (1 - new_hire_termination_rate))
            ...
            all_calculations_correct = all(result["match"] for result in calculation_results)
    ...
    def measure_performance_regression(self, baseline_time: Optional[float] = None) -> Dict[str, Any]:
            ...
            with patch('orchestrator.simulator_pipeline.clean_duckdb_data'):
                with patch('orchestrator.simulator_pipeline._execute_single_year_with_recovery') as mock_execute:
                    ...
                    mock_execute.return_value = YearResult(
                        year=2025, success=True, active_employees=1000,
                        total_terminations=100, experienced_terminations=80,
                        new_hire_terminations=20, total_hires=130,
                        growth_rate=0.03, validation_passed=True
                    )
                    ...
                    regression_check = {}
                    if baseline_time:
                        time_regression = (execution_time - baseline_time) / baseline_time
                        regression_check = {
    ...
    def generate_validation_report(self) -> Dict[str, Any]:
        ...
        overall_success = all(result.get("success", False) for result in all_validations)
        ...
        report = {
            ...
            "s013_epic_status": "✅ VALIDATION PASSED" if overall_success else "❌ VALIDATION FAILED",
            "summary": self._generate_summary(coverage_result, behavior_result, performance_result)
        }

        return report

    def _generate_summary(self, coverage_result, behavior_result, performance_result) -> str:
        """Generate validation summary."""
        lines = ["=== S013-07 Validation Summary ==="]

        # Coverage summary
        coverage_pct = coverage_result.get("coverage_data", {}).get("coverage_percentage", 0)
        coverage_status = "✅" if coverage_result.get("meets_threshold", False) else "❌"
        lines.append(f"{coverage_status} Unit Test Coverage: {coverage_pct:.1f}% (Target: ≥95%)")

        # Behavior summary
        behavior_status = "✅" if behavior_result.get("success", False) else "❌"
        lines.append(f"{behavior_status} Behavior Preservation: {behavior_result.get('summary', 'Unknown')}")

        # Performance summary
        perf_status = "✅" if performance_result.get("success", False) else "❌"
        exec_time = performance_result.get("performance_data", {}).get("execution_time", 0)
        lines.append(f"{perf_status} Performance Validation: {exec_time:.3f}s execution time")
