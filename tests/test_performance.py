...
class TestPerformance:
    ...
    def test_single_year_performance(self):
        ...
        with duckdb.connect(str(get_database_path())) as conn:
            ...
            assert (
                workforce_count > 0
            ), "No workforce snapshot generated in performance test"
    ...
    def test_memory_usage_bounds(self):
        ...
        assert (
            memory_growth < 2048
        ), f"Memory growth {memory_growth:.1f}MB exceeds 2GB limit"
    ...
    def test_multi_year_scalability(self):
        ...
        if two_year_duration > 0:
            ...
            assert (
                scaling_factor < 2.0
            ), f"Poor scaling: 3-year took {scaling_factor:.1f}x longer than 2-year"
    ...
    def test_database_query_performance(self):
        ...
        with duckdb.connect(str(get_database_path())) as conn:
            ...
            assert (
                query_duration < 5.0
            ), f"Basic aggregation query took {query_duration:.2f}s, expected <5s"
            ...
            assert (
                join_duration < 10.0
            ), f"Complex join query took {join_duration:.2f}s, expected <10s"
    ...
    def test_concurrent_read_performance(self):
        ...
        assert (
            total_time < 15.0
        ), f"Concurrent queries took {total_time:.2f}s, expected <15s"
    ...
    def test_large_dataset_handling(self):
        ...
        with duckdb.connect(str(get_database_path())) as conn:
            ...
            if workforce_size > 0:
                time_per_thousand_employees = (
                    complex_query_duration * 1000
                ) / workforce_size
                assert (
                    time_per_thousand_employees < 30.0
                ), f"Complex query performance poor: {time_per_thousand_employees:.2f}s per 1000 employees"
    ...
    def test_disk_io_performance(self):
        ...
        if db_path.exists():
            ...
            assert (
                db_size_mb < 10240
            ), f"Database file too large: {db_size_mb:.1f}MB > 10GB"
            ...
            if db_size_mb > 0:
                ...
                assert (
                    read_time_per_mb < 0.1
                ), f"Database read performance poor: {read_time_per_mb:.3f}s per MB"
    ...
    def test_memory_efficiency_over_time(self):
        ...
        assert (
            memory_growth < 1024
        ), f"Memory grew too much over multiple runs: {memory_growth:.1f}MB"
