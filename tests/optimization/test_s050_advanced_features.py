...
from optimization.ab_testing_framework import (ABTestAnalysis, ABTestConfig,
                                               ABTestMetric,
                                               OptimizationABTestFramework,
                                               TestVariant)
from optimization.business_constraints import (BudgetConstraint,
                                               BusinessConstraintFramework,
                                               ConstraintEvaluationResult,
                                               EquityConstraint,
                                               IndividualRaiseConstraint)
from optimization.configurable_merit_distribution import (
    ConfigurableMeritDistributionSystem, DistributionStrategy,
    LinearMeritDistribution, MeritDistributionConfig, RiskFactor)
from optimization.enhanced_sensitivity_analysis import (
    EnhancedSensitivityAnalyzer, InteractionEffect, SensitivityAnalysisReport,
    SensitivityResult)
from optimization.warm_start_cache import (OptimizationHistoryEntry,
                                           WarmStartCandidate,
...
class TestWarmStartOptimizationCache:
    ...
    def test_parameter_normalization(self, warm_start_cache):
        ...
        normalized = warm_start_cache._normalize_parameter_value(
            "merit_rate_level_1", 0.05
        )
        assert 0 <= normalized <= 1

        # Test unknown parameter
        normalized_unknown = warm_start_cache._normalize_parameter_value(
            "unknown_param", 0.5
        )
    ...
    def test_parameter_similarity_calculation(self, warm_start_cache):
        ...
        similarity_high = warm_start_cache._calculate_parameter_similarity(
            params1, params2
        )
        similarity_low = warm_start_cache._calculate_parameter_similarity(
            params1, params3
        )
...
class TestEnhancedSensitivityAnalyzer:
    ...
    def parameter_bounds(self):
        ...
        return {"param1": (0.01, 0.10), "param2": (0.005, 0.05), "param3": (1.0, 1.5)}

    @pytest.fixture
    def sensitivity_analyzer(
        self, mock_duckdb_resource, mock_objective_function, parameter_bounds
    ):
...
class TestBusinessConstraintFramework:
    ...
    def test_budget_constraint_evaluation(self):
        ...
        constraint = BudgetConstraint(
            max_total_budget=1_000_000, budget_buffer_pct=0.05
        )
    ...
    def test_equity_constraint_evaluation(self):
        ...
        constraint = EquityConstraint(
            max_variance_ratio=0.15, min_level_progression=1.05
        )
...
class TestABTestingFramework:
    ...
    def test_continuous_metric_test(self, ab_framework):
        ...
        test_stat, p_value = ab_framework._continuous_metric_test(
...
class TestS050Integration:
    ...
    def test_end_to_end_optimization_workflow(self, mock_duckdb_resource):
        ...
        constraint_result = constraint_framework.evaluate_all_constraints(
            base_parameters, {"workforce_size": 1000, "avg_salary": 75000}
        )
        ...
        merit_config = MeritDistributionConfig(
            strategy=DistributionStrategy.LINEAR, base_merit_rate=0.04, level_count=3
        )
