...
class TestRegressionSuite:
    ...
    def test_employee_id_uniqueness(self, temp_db):
        ...
        assert result[0] == result[1], f"Duplicate employee_ids in staging: {result[0]} total, {result[1]} unique"
    ...
    def test_event_sourcing_integrity(self, temp_db):
        ...
        assert result[0] == 0, f"Found {result[0]} invalid events missing required fields"
    ...
    def test_simulation_pipeline_integrity(self, temp_db):
        ...
        for year, current, prev, growth in result:
            if prev is not None:
                ...
                assert -0.05 <= growth <= 0.08, f"Year {year}: Growth rate {growth:.3f} outside acceptable range"
    ...
    def test_dbt_snapshot_consistency(self):
        ...
        result = subprocess.run([
            'dbt', 'snapshot', '--select', 'scd_workforce_state',
            '--vars', '{"simulation_year": 2025}',
            '--profiles-dir', '/Users/nicholasamaral/planwise_navigator/dbt'
        ], capture_output=True, text=True, cwd='/Users/nicholasamaral/planwise_navigator/dbt')

        assert result.returncode == 0, f"dbt snapshot failed: {result.stderr}"
        assert "Completed successfully" in result.stdout, f"dbt snapshot didn't complete successfully: {result.stdout}"
    ...
    def test_data_quality_validation(self, temp_db):
        ...
        expected_errors = {'DUPLICATE_IDS'}  # This is expected due to baseline/census overlap

        for check_type, count, desc in result:
            if check_type not in expected_errors:
                pytest.fail(f"Unexpected ERROR-level data quality issue: {check_type} ({count} issues) - {desc}")
