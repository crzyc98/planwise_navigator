...
def benchmark(name: str) -> BenchmarkResult:
    ...
    # CPU monitoring removed for simplicity
    ...
    try:
        ...
        result.memory_peak = max(
            result.memory_start, result.memory_end
        )  # TODO: Track actual peak

        print(
            f"[{name}] Duration: {result.duration:.2f}s, "
        ...
        df = pd.DataFrame(
            {
                ...
                "current_compensation": [
                    40000 + (i % 5) * 20000 for i in range(num_employees)
                ],
...
def benchmark_simulation_year(db_path: str, year: int) -> BenchmarkResult:
    ...
    with benchmark(f"simulation_year_{year}") as result:
        ...
        snapshot_count = conn.execute(
            f"SELECT COUNT(*) FROM workforce_snapshot_{year}"
        ).fetchone()[0]
...
def run_full_benchmark():
    ...
    try:
        ...
        print(
            results_df.groupby("name")[["duration_seconds", "memory_delta_mb"]].mean()
        )
