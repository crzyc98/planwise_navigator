...
from orchestrator_mvp.core.event_emitter import (
    calculate_promotion_probability, generate_promotion_events,
...
class TestPromotionEventsFix:
    ...
    def test_promotion_event_generation_not_zero(
        self, sample_workforce, sample_hazard_config
    ):
        """Test that promotion events are actually generated (not 0 like before the fix)."""
        with patch(
            "orchestrator_mvp.core.event_emitter.load_hazard_configuration"
        ) as mock_load:
            ...
            assert (
                150 < len(events) < 350
            ), f"Unexpected number of promotion events: {len(events)}"
    ...
    def test_hazard_based_calculations(self, sample_hazard_config):
        ...
        prob_level1 = calculate_promotion_probability(
            level=1, tenure=5, age=35, hazard_config=sample_hazard_config
        )
        # Expected: 0.08 * 1.0 * 1.0 * 1.0 = 0.08
        assert (
            abs(prob_level1 - 0.08) < 0.001
        ), f"Level 1 probability incorrect: {prob_level1}"

        # Test Level 2 employee
        prob_level2 = calculate_promotion_probability(
            level=2, tenure=5, age=35, hazard_config=sample_hazard_config
        )
        # Expected: 0.07 * 1.0 * 1.0 * 0.9 = 0.063
        assert (
            abs(prob_level2 - 0.063) < 0.001
        ), f"Level 2 probability incorrect: {prob_level2}"

        # Test Level 3 employee
        prob_level3 = calculate_promotion_probability(
            level=3, tenure=5, age=35, hazard_config=sample_hazard_config
        )
        # Expected: 0.05 * 1.0 * 1.0 * 0.8 = 0.04
        assert (
            abs(prob_level3 - 0.04) < 0.001
        ), f"Level 3 probability incorrect: {prob_level3}"

        # Test Level 4 employee
        prob_level4 = calculate_promotion_probability(
            level=4, tenure=5, age=35, hazard_config=sample_hazard_config
        )
        # Expected: 0.04 * 1.0 * 1.0 * 0.7 = 0.028
        assert (
            abs(prob_level4 - 0.028) < 0.001
        ), f"Level 4 probability incorrect: {prob_level4}"
    ...
    def test_random_value_distribution(self):
        ...
        assert (
            0.45 < np.mean(random_values) < 0.55
        ), f"Random mean not centered: {np.mean(random_values)}"

        # Should have reasonable spread
        assert (
            np.std(random_values) > 0.25
        ), f"Random values not well distributed: std={np.std(random_values)}"
    ...
    def test_workforce_source_validation(self, sample_workforce, sample_hazard_config):
        ...
        with patch(
            ...
            assert promoted_ids.issubset(
                workforce_ids
            ), "Promoted employees not from input workforce"
    ...
    def test_promotion_rates_by_level(self, sample_workforce, sample_hazard_config):
        ...
        with patch(
            ...
            assert (
                0.05 < promotion_rates.get(1, 0) < 0.11
            ), f"Level 1 rate out of range: {promotion_rates.get(1, 0)}"

            # Level 2: ~5-7% (base_rate 0.07 * dampener 0.9 = 0.063)
            assert (
                0.04 < promotion_rates.get(2, 0) < 0.09
            ), f"Level 2 rate out of range: {promotion_rates.get(2, 0)}"

            # Level 3: ~4-6% (base_rate 0.05 * dampener 0.8 = 0.04)
            assert (
                0.02 < promotion_rates.get(3, 0) < 0.07
            ), f"Level 3 rate out of range: {promotion_rates.get(3, 0)}"

            # Level 4: ~3-4% (base_rate 0.04 * dampener 0.7 = 0.028)
            assert (
                0.01 < promotion_rates.get(4, 0) < 0.05
            ), f"Level 4 rate out of range: {promotion_rates.get(4, 0)}"
    ...
    def test_event_structure_validation(self, sample_workforce, sample_hazard_config):
        ...
        with patch(
            ...
            for event in events:
                ...
                ), f"Missing required fields in event: {event}"

                # Validate event type
                assert (
                    event["event_type"] == "promotion"
                ), f"Wrong event type: {event['event_type']}"

                # Validate level progression
                assert (
                    event["new_level"] == event["old_level"] + 1
                ), "Invalid level progression"

                # Validate salary increase
                assert (
                    event["new_salary"] > event["old_salary"]
                ), "Salary should increase with promotion"

                # Validate promotion percentage is reasonable (10-20%)
                assert (
                    0.10 <= event["promotion_percentage"] <= 0.20
                ), f"Invalid promotion percentage: {event['promotion_percentage']}"
    ...
    def test_database_storage_validation(self, sample_workforce, sample_hazard_config):
        ...
        with patch(
            ...
            assert (
                events_df["employee_id"].dtype == "object"
            ), "employee_id should be string"
            assert events_df["simulation_year"].dtype in [
                "int64",
                "int32",
            ], "simulation_year should be integer"
            assert events_df["old_level"].dtype in [
                "int64",
                "int32",
            ], "old_level should be integer"
            assert events_df["new_level"].dtype in [
                "int64",
                "int32",
            ], "new_level should be integer"
            assert events_df["old_salary"].dtype in [
                "float64",
                "int64",
            ], "old_salary should be numeric"
            assert events_df["new_salary"].dtype in [
                "float64",
                "int64",
            ], "new_salary should be numeric"

            # Validate no null values in required fields
            assert (
                not events_df[["event_id", "employee_id", "event_type", "event_date"]]
                .isnull()
                .any()
                .any()
            ...
            assert events_df["event_id"].nunique() == len(
                events_df
