...
from orchestrator.utils.cold_start_validation import (
    check_schema_compatibility, validate_workforce_initialization)
...
def test_idempotency_first_year(duckdb_resource):
    ...
    with duckdb_resource.get_connection() as conn:
        ...
        first_count = conn.execute(
            "SELECT COUNT(*) FROM fct_workforce_snapshot WHERE simulation_year = 1"
        ).fetchone()[0]
...
def test_workforce_continuity_validation(duckdb_resource):
    ...
    continuity_check = next(
        c for c in result["checks"] if c["check_name"] == "workforce_continuity"
    )
...
def test_simulation_run_log_tracking(duckdb_resource):
    ...
    run_log_check = next(
        c for c in result["checks"] if c["check_name"] == "simulation_run_logged"
    )
