...
from streamlit_dashboard.optimization_schemas import (
    ...
    validate_parameters, assess_parameter_risk
)
...
from orchestrator.optimization.optimization_schemas import (
    OptimizationRequest, OptimizationResult, OptimizationCache
)
...
class TestAlgorithmPerformance:
    ...
    def setup_method(self):
        ...
        self.mock_conn.execute.return_value.fetchall.return_value = [
            (1, 50000, 2500), (2, 60000, 3000), (3, 70000, 3500)
        ]
    ...
    def test_optimization_convergence_speed(self):
        ...
        assert large_time < small_time * 10, f"Large optimization ({large_time:.3f}s) too slow vs small ({small_time:.3f}s)"
    ...
    def test_objective_function_performance(self):
        ...
        for i, params in enumerate(test_params):
            ...
            with self.profiler.profile():
                ...
                with patch.object(obj_funcs, '_update_parameters') as mock_update:
                    ...
                    for _ in range(10):
                        ...
                        combined = obj_funcs.combined_objective(params, {"cost": 0.5, "equity": 0.3, "targets": 0.2})
            ...
            avg_time_per_calc = self.profiler.metrics.execution_time / 40  # 10 iterations * 4 calculations
            assert avg_time_per_calc < 0.1, f"Objective function {complexity} too slow: {avg_time_per_calc:.3f}s per call"
    ...
    def test_sensitivity_analysis_performance(self):
        ...
        for size_name, params in test_cases:
            ...
            assert call_count[0] >= expected_calls, f"Expected at least {expected_calls} calls, got {call_count[0]}"
    ...
    def test_parameter_validation_performance(self):
        ...
        assert self.profiler.metrics.memory_usage_mb < 100, f"Memory usage too high: {self.profiler.metrics.memory_usage_mb}MB"

        # Average validation time
        avg_validation_time = self.profiler.metrics.execution_time / 100
        assert avg_validation_time < 0.05, f"Average validation time too high: {avg_validation_time:.3f}s"
...
class TestMemoryUsagePatterns:
    ...
    def test_memory_leak_detection(self):
        ...
        for iteration in range(50):
            ...
            recovered_params = schema.transform_from_compensation_tuning_format(comp_format)
        ...
        if len(memory_samples) >= 3:
            ...
            assert final_memory < mid_memory * 1.5, "Memory usage appears to be growing continuously"
    ...
    def test_large_parameter_set_memory_usage(self):
        ...
        assert self.profiler.metrics.memory_usage_mb < 200, f"Memory usage too high: {self.profiler.metrics.memory_usage_mb}MB"
        assert self.profiler.metrics.execution_time < 10.0, f"Execution time too high: {self.profiler.metrics.execution_time}s"
    ...
    def test_memory_efficient_data_structures(self):
        ...
        operations = [
            ...
            ("parameter_validation", lambda: schema.validate_parameter_set(get_default_parameters())),
            ("format_transformation", lambda: schema.transform_to_compensation_tuning_format(get_default_parameters()))
        ]
        ...
        for op_name, usage in memory_usage.items():
            assert usage < 50, f"Operation {op_name} uses too much memory: {usage:.1f}MB"
...
class TestScalabilityPerformance:
    ...
    def test_workforce_size_scalability(self):
        ...
        for size in workforce_sizes:
            ...
            with self.profiler.profile():
                ...
                avg_by_level = workforce_data.groupby('job_level')['current_compensation'].mean()

                # Simulate parameter impact calculations
                for level in [1, 2, 3, 4, 5]:
                    level_data = workforce_data[workforce_data['job_level'] == level]
                    if len(level_data) > 0:
                        impact = level_data['current_compensation'].sum() * 0.045  # 4.5% merit
            ...
            if size == 100:
                ...
                assert self.profiler.metrics.execution_time < baseline_time * scale_factor * 2
                # Memory should scale roughly linearly but not exceed reasonable limits
                assert self.profiler.metrics.memory_usage_mb < baseline_memory * scale_factor * 3
    ...
    def _generate_workforce_data(self, size: int) -> pd.DataFrame:
        ...
        for i in range(size):
            employee = {
                ...
                'department': np.random.choice(['Engineering', 'Finance', 'HR', 'Sales'])
            }
    ...
    def test_parameter_space_scalability(self):
        ...
        for count in scenario_counts:
            ...
            assert self.profiler.metrics.execution_time < count * 0.1, f"Too slow for {count} scenarios"
            assert self.profiler.metrics.memory_usage_mb < count * 2, f"Too much memory for {count} scenarios"
    ...
    def test_concurrent_scalability(self):
        ...
        for worker_count in worker_counts:
            ...
            assert all(result == 10 for result in results)  # Each worker completed 10 tasks
...
class TestCachingPerformance:
    ...
    def test_parameter_validation_caching(self):
        ...
        assert warm_cache_time <= cold_cache_time * 1.5, "Warm cache performance regression"
    ...
    def test_memory_efficient_caching(self):
        ...
        assert memory_increase < 100, f"Cache uses too much memory: {memory_increase:.1f}MB"
...
class TestDatabasePerformance:
    ...
    def test_database_query_performance(self):
        ...
        avg_query_time = self.profiler.metrics.execution_time / self.profiler.metrics.database_queries
    ...
    def test_connection_management_performance(self):
        ...
        components = [
            ObjectiveFunctions(self.mock_duckdb, f"perf_test_{i}")
            for i in range(5)
        ]
        ...
        assert connection_count[0] <= len(components) * 2, "Too many connections created"
        assert self.profiler.metrics.execution_time < 2.0, "Connection management too slow"


if __name__ == "__main__":
    # Run performance tests
    pytest.main([
        __file__ + "::TestAlgorithmPerformance::test_optimization_convergence_speed",
