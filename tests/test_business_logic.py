...
class TestBusinessLogic:
    ...
    def test_deterministic_simulation(self):
        ...
        assert (
            workforce_count_1 == workforce_count_2
        ), f"Workforce count mismatch: {workforce_count_1} vs {workforce_count_2}"
        assert (
            abs((total_compensation_1 or 0) - (total_compensation_2 or 0)) < 0.01
        ), f"Total compensation mismatch: {total_compensation_1} vs {total_compensation_2}"
        assert (
            abs((total_contributions_1 or 0) - (total_contributions_2 or 0)) < 0.01
        ), f"Total contributions mismatch: {total_contributions_1} vs {total_contributions_2}"
        assert (
            hire_events_1 == hire_events_2
        ), f"Hire events mismatch: {hire_events_1} vs {hire_events_2}"
        ...
        with duckdb.connect("simulation.duckdb") as conn:
            ...
            if len(growth_data) >= 2:
                ...
                if year_2025_count > 0:
                    growth_achieved = (
                        year_2026_count - year_2025_count
                    ) / year_2025_count

                    # Allow reasonable growth range (-5% to +15%)
                    assert (
                        -0.05 <= growth_achieved <= 0.15
                    ), f"Growth rate {growth_achieved:.2%} outside reasonable range"
    ...
    def test_compensation_reasonableness(self):
        ...
        with duckdb.connect("simulation.duckdb") as conn:
            ...
            if comp_stats and comp_stats[4] > 0:  # If we have data
                ...
                assert (
                    min_comp >= 25000
                ), f"Minimum compensation too low: ${min_comp:,.2f}"
                assert (
                    max_comp <= 750000
                ), f"Maximum compensation too high: ${max_comp:,.2f}"
                assert (
                    40000 <= avg_comp <= 150000
                ), f"Average compensation unreasonable: ${avg_comp:,.2f}"
                ...
                if stddev_comp:
                    ...
                    assert (
                        0.1 <= cv <= 1.0
                    ), f"Compensation distribution too {('narrow' if cv < 0.1 else 'wide')}: CV = {cv:.2f}"
    ...
    def test_contribution_rates_realistic(self):
        ...
        with duckdb.connect("simulation.duckdb") as conn:
            ...
            for rate, count in deferral_data:
                assert (
                    0 <= rate <= 0.5
                ), f"Unrealistic deferral rate: {rate:.2%} for {count} employees"
            ...
            if avg_deferral:
                assert (
                    0.02 <= avg_deferral <= 0.20
                ), f"Average deferral rate unrealistic: {avg_deferral:.2%}"
        ...
        with duckdb.connect("simulation.duckdb") as conn:
            ...
            if monthly_events:
                ...
                for month, count in monthly_events:
                    ...
                    assert (
                        month_pct <= 0.5
                    ), f"Month {month} has {month_pct:.2%} of events - too concentrated"
        ...
        with duckdb.connect("simulation.duckdb") as conn:
            ...
            if event_balance:
                ...
                if hires > 0 and terminations > 0:
                    ...
                    assert (
                        0.3 <= hire_term_ratio <= 5.0
                    ), f"Hire/termination ratio unrealistic: {hire_term_ratio:.2f}"
    ...
    def test_promotion_progression_logic(self):
        ...
        with duckdb.connect("simulation.duckdb") as conn:
            ...
            assert (
                orphaned_promotions == 0
            ), f"Found {orphaned_promotions} promotions for non-existent employees"
            ...
            for emp_id, new_comp, prev_comp, raise_pct in promotion_raises:
                ...
                assert (
                    0.02 <= raise_pct <= 0.50
                ), f"Employee {emp_id} promotion raise {raise_pct:.2%} outside reasonable range"
    ...
    def test_merit_increase_logic(self):
        ...
        with duckdb.connect("simulation.duckdb") as conn:
            ...
            for emp_id, new_comp, prev_comp, raise_pct in merit_raises:
                ...
                assert (
                    0.0 <= raise_pct <= 0.15
                ), f"Employee {emp_id} merit raise {raise_pct:.2%} outside reasonable range"
    ...
    def test_workforce_demographic_consistency(self):
        ...
        with duckdb.connect("simulation.duckdb") as conn:
            ...
            if tenure_stats and tenure_stats[0] is not None:
                ...
                assert (
                    2 <= avg_tenure <= 20
                ), f"Average tenure unrealistic: {avg_tenure}"
