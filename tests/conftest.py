...
from streamlit_dashboard.optimization_schemas import (get_default_parameters,
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "edge_case: Edge case and boundary testing")
    config.addinivalue_line(
    ...
    class PerformanceTracker:
        ...
        def assert_performance(self, max_time=None, max_memory=None):
            ...
            if max_memory and self.metrics["memory_delta"] > max_memory:
                pytest.fail(
                    f"Memory usage {self.metrics['memory_delta']:.1f}MB exceeds limit {max_memory}MB"
                )
...
class TestDataGenerator:
    ...
    def parameter_variations(
        base_params: Dict[str, float], count: int = 10
    ) -> List[Dict[str, float]]:
...
def benchmark_baseline():
    ...
    return {
        ...
        "simulation_pipeline": {"max_time": 30.0, "max_memory": 200.0},
    }
...
def assert_performance_acceptable(metrics, benchmarks):
    ...
    if metrics["execution_time"] > benchmarks.get("max_time", float("inf")):
        pytest.fail(
            f"Execution time {metrics['execution_time']:.2f}s exceeds benchmark {benchmarks['max_time']}s"
        )

    if metrics.get("memory_delta", 0) > benchmarks.get("max_memory", float("inf")):
        pytest.fail(
            f"Memory usage {metrics['memory_delta']:.1f}MB exceeds benchmark {benchmarks['max_memory']}MB"
        )
    ...
    class ProductionSimulator:
        ...
        def get_run_stats(self):
            ...
            return {
                ...
                "success_rate": sum(1 for run in self.runs if run["success"])
                / len(self.runs),
            ...
            try:
                with duckdb.connect(str(get_database_path())) as conn:
                    if check_type == "no_nulls":
                        ...
                        passed = null_count == 0
                        self.checks.append(
                            ("no_nulls", f"{table_name}.{column}", passed, null_count)
                        )
                        return passed
                        ...
                        passed = (min_val is None or actual_min >= min_val) and (
                            max_val is None or actual_max <= max_val
                        )
                        self.checks.append(
                            (
                                "value_range",
