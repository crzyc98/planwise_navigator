...
class EventDataGenerator:
    ...
    @staticmethod
    def generate_workforce_events(
        num_employees: int, years: int = 5
        ...
        for employee_idx in range(num_employees):
            ...
            for year_offset in range(years):
                ...
                if employee_idx % 5 != 0:  # 80% get merit
                    events.append(
                        EventFactory.create_merit_event(
                            employee_id=employee_id,
                            scenario_id=scenario_id,
                            plan_design_id=plan_design_id,
            ...
            if employee_idx % 7 == 0:  # ~15% termination rate
                ...
                if term_year <= 2024:
                    events.append(
                        EventFactory.create_termination_event(
                            employee_id=employee_id,
                            scenario_id=scenario_id,
                            plan_design_id=plan_design_id,
                            termination_reason="voluntary",
                            effective_date=date(term_year, 6, 15),
                            final_compensation=Decimal(
                                f"{55000 + (employee_idx % 45000)}"
                            ),
                        )
                    )
    ...
    @staticmethod
    def generate_dc_plan_events(
        num_participants: int, years: int = 5
    ) -> List[SimulationEvent]:
        ...
        for participant_idx in range(num_participants):
            ...
            if participant_idx % 20 != 0:  # 95% enrollment
                ...
                events.append(
                    EnrollmentEventFactory.create_enrollment_event(
                        employee_id=employee_id,
                        plan_id=plan_id,
                        scenario_id=scenario_id,
                        plan_design_id=plan_design_id,
                ...
                for year_offset in range(years):
                    ...
                    for month in range(1, 13):
                        ...
                        employer_match = min(
                            employee_contrib, monthly_compensation * Decimal("0.03")
                        )
                        ...
                        events.append(
                            ContributionEventFactory.create_contribution_event(
                                ...
                                vesting_service_years=Decimal(
                                    f"{year_offset}.{month:02d}"
                                ),
                ...
                for year_offset in range(1, years + 1):
                    ...
                    vesting_percentage = min(
                        Decimal("1.0"), Decimal(f"{year_offset * 0.2}")
                    )  # 20% per year
                    events.append(
                        VestingEventFactory.create_vesting_event(
                            employee_id=employee_id,
                            plan_id=plan_id,
                            scenario_id=scenario_id,
                            plan_design_id=plan_design_id,
...
class TestEventSchemaPerformance:
    ...
    def test_bulk_event_ingest_performance(self):
        ...
        ), f"Ingest too slow: {events_per_second:.0f}/sec < 50K/sec target"
        assert (
            metrics["memory_delta_mb"] < 2000
    ...
    def test_history_reconstruction_performance(self):
        ...
        ), f"History reconstruction too slow: {metrics['duration_seconds']:.3f}s > 5s target"

        # Verify reconstruction quality
        assert (
            len(reconstruction_queries) == 100
        ), "Should have reconstructed 100 participant histories"
    ...
    def test_schema_validation_performance(self):
        ...
        for i in range(50):
            ...
            # Forfeiture event
            test_events.append(
                PlanAdministrationEventFactory.create_forfeiture_event(
                    employee_id=employee_id,
                    plan_id=plan_id,
                    scenario_id=scenario_id,
                    plan_design_id=plan_design_id,
        ...
        print(
            f"Validations/second: {len(test_events)/metrics['duration_seconds']:,.0f}"
        )

        # Performance targets
        assert (
            avg_validation_time < 10.0
        ), f"Avg validation too slow: {avg_validation_time:.3f}ms > 10ms target"
        assert (
            max_validation_time < 50.0
        ), f"Max validation too slow: {max_validation_time:.3f}ms > 50ms acceptable"
    ...
    def test_memory_efficiency_simulation(self):
        ...
        for batch_idx in range(10):  # 10 batches of 10K employees each
            batch_events = EventDataGenerator.generate_workforce_events(
                batch_size, years=2
            )
            ...
            print(
                f"Batch {batch_idx + 1}/10: {len(batch_events):,} events, Memory: {current_memory:.2f}GB"
            )
        ...
        assert (
            peak_memory_gb < 8.0
        ), f"Memory usage too high: {peak_memory_gb:.2f}GB > 8GB target"

        # Verify data quality
        assert (
            len(all_events) >= 400000
        ), f"Should generate 400K+ events for 100K employees"  # ~4 events per employee minimum
