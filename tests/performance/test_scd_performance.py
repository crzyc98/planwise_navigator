...
def test_scd_processing_performance(
    duckdb_resource, dbt_resource, large_workforce_dataset
):
    ...
    try:
        ...
        assert (
            execution_time < 120
        ), f"SCD processing took {execution_time:.2f}s, expected <120s"
...
def test_scd_memory_usage(duckdb_resource, dbt_resource, large_workforce_dataset):
    ...
    try:
        ...
        assert (
            memory_used < 4096
        ), f"SCD processing used {memory_used:.2f}MB, expected <4096MB"
...
def test_scd_throughput_benchmark(duckdb_resource, dbt_resource):
    ...
    for result in results:
        ...
        ), f"Throughput {result['throughput']:.0f} below target of 1000 records/sec"
...
def test_scd_scalability(duckdb_resource, dbt_resource):
    ...
    for size in [10000, 50000, 100000]:
        ...
        try:
            ...
            assert (
                execution_time < 300
            ), f"Processing {size} records took {execution_time:.2f}s, expected <300s"
            assert (
                throughput >= 500
            ), f"Throughput {throughput:.0f} below minimum of 500 records/sec"

            print(
                f"✓ Scalability test {size} records: {execution_time:.2f}s, {throughput:.0f} records/sec"
            )
