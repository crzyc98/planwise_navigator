...
class TestRatioTestPass:
    ...
    def test_ratio_test_pass(self, service_with_db):
        ...
        _insert_employee(
            conn, "HCE1", 2025, 100000.0, 100000.0, core_amount=8000.0, tenure=10.0
        )
        # NHCE: NEC=6000, comp=100000 -> rate=6%
        _insert_employee(conn, "NHCE1", 2024, 80000.0, 80000.0, 0.0)
        _insert_employee(
            conn, "NHCE1", 2025, 100000.0, 100000.0, core_amount=6000.0, tenure=5.0
        )
...
class TestRatioFailGeneralPass:
    ...
    def test_general_test_fallback_pass(self, service_with_db):
        ...
        _insert_employee(conn, "HCE1", 2024, 200000.0, 200000.0, 0.0)
        _insert_employee(
            conn, "HCE1", 2025, 100000.0, 100000.0, core_amount=10000.0, tenure=10.0
        )
        # HCE2: NEC=10000, comp=100000 -> 10%
        _insert_employee(conn, "HCE2", 2024, 200000.0, 200000.0, 0.0)
        _insert_employee(
            conn, "HCE2", 2025, 100000.0, 100000.0, core_amount=10000.0, tenure=8.0
        )
        # HCE median = 10%
        ...
        _insert_employee(conn, "NHCE1", 2024, 80000.0, 80000.0, 0.0)
        _insert_employee(
            conn, "NHCE1", 2025, 100000.0, 100000.0, core_amount=1000.0, tenure=3.0
        )
        # NHCE2: NEC=8000, comp=100000 -> 8%
        _insert_employee(conn, "NHCE2", 2024, 80000.0, 80000.0, 0.0)
        _insert_employee(
            conn, "NHCE2", 2025, 100000.0, 100000.0, core_amount=8000.0, tenure=4.0
        )
        # NHCE3: NEC=9000, comp=100000 -> 9%
        _insert_employee(conn, "NHCE3", 2024, 80000.0, 80000.0, 0.0)
        _insert_employee(
            conn, "NHCE3", 2025, 100000.0, 100000.0, core_amount=9000.0, tenure=5.0
        )
...
class TestBothTestsFail:
    ...
    def test_both_tests_fail(self, service_with_db):
        ...
        _insert_employee(conn, "NHCE1", 2024, 80000.0, 80000.0, 0.0)
        _insert_employee(
            conn, "NHCE1", 2025, 100000.0, 100000.0, core_amount=2000.0, tenure=3.0
        )
...
class TestServiceRiskFlag:
    ...
    def test_service_risk_flag_triggered(self, service_with_db):
        ...
        service.storage.get_merged_config = MagicMock(
            return_value={"employer_core_contribution": {"status": "graded_by_service"}}
        )
    ...
    def test_nec_only_excludes_match(self, service_with_db):
        ...
        with patch("duckdb.connect", return_value=conn):
            result = service.run_401a4_test(
                "ws1", "sc1", "Test", 2025, include_employees=True, include_match=False
            )
...
class TestAllHCE:
    ...
    def test_all_hce_population(self, service_with_db):
        ...
        _insert_employee(conn, "HCE1", 2024, 200000.0, 200000.0, 0.0)
        _insert_employee(
            conn, "HCE1", 2025, 200000.0, 200000.0, core_amount=10000.0, tenure=10.0
        )
        _insert_employee(conn, "HCE2", 2024, 250000.0, 250000.0, 0.0)
        _insert_employee(
            conn, "HCE2", 2025, 250000.0, 250000.0, core_amount=12500.0, tenure=8.0
        )
        ...
        assert (
            "nhce" in result.test_message.lower()
            or "no nhce" in result.test_message.lower()
        )
...
class TestAllNHCE:
    ...
    def test_all_nhce_auto_pass(self, service_with_db):
        ...
        _insert_employee(conn, "NHCE1", 2024, 80000.0, 80000.0, 0.0)
        _insert_employee(
            conn, "NHCE1", 2025, 80000.0, 80000.0, core_amount=4000.0, tenure=5.0
        )
        _insert_employee(conn, "NHCE2", 2024, 70000.0, 70000.0, 0.0)
        _insert_employee(
            conn, "NHCE2", 2025, 70000.0, 70000.0, core_amount=3500.0, tenure=3.0
        )
...
class TestNonBenefitingExcluded:
    ...
    def test_match_only_employee_included(self, service_with_db):
        ...
        with patch("duckdb.connect", return_value=conn):
            result = service.run_401a4_test(
                "ws1", "sc1", "Test", 2025, include_match=True
            )
