...
class TestRegulatory:
    ...
    def test_irs_contribution_limits_2025(self, db_connection):
        ...
        assert (
            len(violations) == 0
        ), f"Found {len(violations)} employees exceeding 2025 IRS limits"
    ...
    def test_irs_contribution_limits_future_years(self, db_connection):
        ...
        if total_contributors_2026 > 0:
            ...
            assert (
                violation_rate < 0.01
            ), f"High IRS limit violation rate in 2026: {violation_rate:.2%}"
    ...
    def test_catch_up_contributions(self, db_connection):
        ...
        if employees_50_plus > 10:  # Only test if we have sufficient 50+ population
            assert (
                len(catch_up_eligible) > 0
            ), "No catch-up contributions found for 50+ employees"
    ...
    def test_catch_up_limits_not_exceeded(self, db_connection):
        ...
        assert (
            len(violations) == 0
        ), f"Found {len(violations)} employees exceeding catch-up limits"
    ...
    def test_compensation_percentage_limits(self, db_connection):
        ...
        assert (
            len(violations) == 0
        ), f"Found {len(violations)} employees contributing >100% of compensation"
    ...
    def test_highly_compensated_employee_limits(self, db_connection):
        ...
        if hce_avg_deferral and nhce_avg_deferral and nhce_avg_deferral > 0:
            ...
            assert (
                ratio <= 2.0
            ), f"HCE deferral rate too high relative to NHCE: {ratio:.2f}x"
    ...
    def test_plan_year_consistency(self, db_connection):
        ...
        if total_events > 0:
            ...
            assert (
                misattribution_rate < 0.05
            ), f"High plan year misattribution rate: {misattribution_rate:.2%}"
    ...
    def test_non_discrimination_testing_data_quality(self, db_connection):
        ...
        assert (
            "HCE" in group_types or "NHCE" in group_types
        ), "Missing compensation groups for non-discrimination testing"
    ...
    def test_regulatory_reporting_data_completeness(self, db_connection):
        ...
        ), f"Found {missing_compensation} active employees with missing/invalid compensation"
        ...
        if total_active > 0:
            ...
            assert (
                missing_rate < 0.8
            ), f"Too many active employees missing contribution data: {missing_rate:.2%}"
