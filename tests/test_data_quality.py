...
class TestDataQuality:
    ...
    def test_contribution_limits_compliance(self, db_connection):
        ...
        assert (
            len(violations) == 0
        ), f"Found {len(violations)} contribution > compensation violations"
    ...
    def test_workforce_growth_consistency(self, db_connection):
        ...
        for year, count, prev_count in growth_data:
            if prev_count:  # Skip first year
                ...
                assert (
                    -0.2 <= growth_rate <= 0.2
                ), f"Unrealistic growth rate in {year}: {growth_rate:.2%}"
    ...
    def test_event_sequence_logic(self, db_connection):
        ...
        assert (
            len(multiple_hires) == 0
        ), f"Found {len(multiple_hires)} employees with multiple hire events"
    ...
    def test_compensation_consistency(self, db_connection):
        ...
        assert (
            negative_comp == 0
        ), f"Found {negative_comp} employees with negative compensation"
        ...
        total_employees = db_connection.execute(
            "SELECT COUNT(*) FROM fct_workforce_snapshot"
        ).fetchone()[0]
        if total_employees > 0:
            ...
            assert (
                high_comp_pct < 0.05
            ), f"Too many high earners: {high_comp_pct:.2%} > 5%"
        ...
        assert (
            very_low_comp == 0
        ), f"Found {very_low_comp} active employees with very low compensation"
    ...
    def test_employee_id_consistency(self, db_connection):
        ...
        assert (
            orphaned_contributions == 0
        ), f"Found {orphaned_contributions} orphaned employee contributions"
        ...
        total_events = db_connection.execute(
        ...
        if total_events > 0:
            ...
            assert (
                orphaned_pct < 0.1
            ), f"Too many orphaned events: {orphaned_pct:.2%} > 10%"
    ...
    def test_date_consistency(self, db_connection):
        ...
        assert (
            future_events == 0
        ), f"Found {future_events} events with unrealistic future dates"
        ...
        assert (
            very_old_events == 0
        ), f"Found {very_old_events} events with unrealistic old dates"
        ...
        assert (
            enrollment_before_hire == 0
        ), f"Found {enrollment_before_hire} enrollments before hire date"
    ...
    def test_numerical_precision(self, db_connection):
        ...
        assert (
            excessive_precision == 0
        ), f"Found {excessive_precision} compensation values with > 2 decimal places"
    ...
    def test_enrollment_status_consistency(self, db_connection):
        ...
        if total_with_enrollment_events > 0:
            inconsistency_rate = (
                missing_enrollment_status / total_with_enrollment_events
            )
            assert (
                inconsistency_rate < 0.1
            ), f"High enrollment status inconsistency: {inconsistency_rate:.2%}"
    ...
    def test_termination_logic(self, db_connection):
        ...
        assert (
            post_termination_activity == 0
        ), f"Found {post_termination_activity} events after termination"
        ...
        assert (
            active_after_termination == 0
        ), f"Found {active_after_termination} active employees after termination"
    ...
    def test_simulation_year_consistency(self, db_connection):
        ...
        if total_events > 0:
            ...
            assert (
                mismatch_rate < 0.05
            ), f"High event year mismatch rate: {mismatch_rate:.2%}"
