...
from risk_assessment import (ComprehensiveRiskAssessment, RiskAssessmentEngine,
                             RiskCategory, RiskLevel, create_risk_dashboard,
...
# Header
st.markdown(
    '<h1 class="main-header">💰 Compensation Tuning Interface</h1>',
    unsafe_allow_html=True,
)
...
def load_current_parameters():
    ...
    try:
        comp_levers_path = (
            Path(__file__).parent.parent.parent / "dbt" / "seeds" / "comp_levers.csv"
        )
...
def load_simulation_results(status_filter=["continuous_active", "new_hire_active"]):
    ...
    if (
        hasattr(st.session_state, "simulation_results")
        and st.session_state.simulation_results
    ):
    ...
    try:
        ...
        try:
            ...
            baseline_avg_salary = (
                baseline_result[1] if baseline_result and baseline_result[1] else 0
            )
        except Exception:
            ...
            if result:
                ...
                if i == 0:
                    ...
                    if baseline_avg_salary > 0:
                        growth_rate = (
                            (avg_salary - baseline_avg_salary) / baseline_avg_salary
                        ) * 100
                    else:
                        growth_rate = 0
                else:
                    ...
                    if previous_salary > 0:
                        growth_rate = (
                            (avg_salary - previous_salary) / previous_salary
                        ) * 100
                    else:
                        growth_rate = 0
...
def validate_parameters(params):
    ...
    if "cola_rate" in params:
        ...
        if cola_avg < 0.01:
            warnings.append(
                f"COLA rate {cola_avg:.1%} is below 1% - may hurt retention"
            )
    ...
    if "merit_base" in params:
        ...
        if merit_avg > 0.08:
            warnings.append(
                f"Merit rate {merit_avg:.1%} is above 8% - may exceed budget"
            )
        if merit_avg < 0.02:
            warnings.append(
                f"Merit rate {merit_avg:.1%} is below 2% - may hurt performance"
            )
    ...
    if "new_hire_salary_adjustment" in params:
        ...
        if adj_avg > 1.3:
            warnings.append(
                f"New hire adjustment {adj_avg:.1%} is above 30% - may create inequity"
            )
        if adj_avg < 1.0:
...
def get_parameter_sensitivity_summary(year_params):
    ...
    current_merit = np.mean(
        [year_params.get("merit_base", {}).get(i, 0.03) for i in range(1, 6)]
    )
...
def validate_synthetic_vs_real(synthetic_results, real_results, tolerance=0.1):
    ...
    try:
        ...
        if (
            synthetic_results.get("growth_rates")
            and real_results.get("growth_rates")
            and len(synthetic_results["growth_rates"])
            == len(real_results["growth_rates"])
        ):
            ...
            if mape > tolerance * 100:  # Convert to percentage
                ...
                validation["recommendations"].append(
                    f"Growth rate difference ({mape:.1f}%) exceeds tolerance"
                )
        ...
        if synthetic_results.get("avg_salaries") and real_results.get("avg_salaries"):
            ...
            if salary_diff > tolerance * 100:
                validation["valid"] = False
                validation["recommendations"].append(
                    f"Final salary difference ({salary_diff:.1f}%) exceeds tolerance"
                )

        # Add recommendations based on validation
        if validation["valid"]:
            validation["recommendations"].append(
                "Synthetic results are well-aligned with real simulation"
            )
        else:
    ...
    try:
            ...
            if param_name in validation_rules:
                ...
                for level, value in param_values.items():
                    ...
                    if current_params and param_name in current_params:
                        current_value = current_params[param_name].get(
                            level, rules["min"]
                        )
        ...
        if parameter_changes and current_params:
            risk_assessment = risk_engine.calculate_overall_risk_assessment(
                parameter_changes=parameter_changes, implementation_timeline=90
            )
        ...
        budget_warnings = check_budget_impact_thresholds(
...
def validate_parameter_consistency(proposed_params):
    ...
    if "merit_base" in proposed_params:
        ...
        if inversions > 2:  # Allow some flexibility but flag major inversions
            issues.append(
                "Merit rates may be inverted - higher levels typically receive lower merit percentages"
            )
    ...
    if "cola_rate" in proposed_params:
        ...
        if len(set(cola_values)) > 1:
            issues.append(
                "COLA rates vary across levels - typically COLA should be uniform"
            )

    # Check promotion probability progression
    if "promotion_probability" in proposed_params:
        promo_values = [
            proposed_params["promotion_probability"].get(i, 0) for i in range(1, 6)
        ]
        ...
        for i in range(1, len(promo_values)):
            if promo_values[i] > promo_values[i - 1] * 1.5:  # Allow some variance
                issues.append(
                    f"Promotion probability for Level {i+1} seems high compared to Level {i}"
                )
...
def generate_risk_mitigation_plan(risk_assessment):
    ...
    category_risks = {}
    all_risks = risk_assessment.aggregate_risks + [
        rf for pr in risk_assessment.parameter_risks for rf in pr.risk_factors
    ]
...
def run_synthetic_simulation(proposed_params, years, baseline_data=None):
    ...
    try:
        ...
        if baseline_data is None:
            ...
            current_results = load_simulation_results(
                ["continuous_active", "new_hire_active"]
            )
        ...
        for i, year in enumerate(years):
            ...
            if i == 0:
                ...
                compensation_factor = (
                    cola_rate + avg_merit
                ) / 0.065  # Normalize around typical 6.5%
                headcount_growth_rate = (
                    0.02 * compensation_factor
                )  # Base 2% growth, adjusted
                ...
                workforce_growth = (
                    (current_headcount - baseline_headcount) / baseline_headcount
                ) * 100
                salary_growth = (
                    (current_salary - baseline_salary) / baseline_salary
                ) * 100
            ...
            status_breakdown = {
                "continuous_active": int(
                    total_employees * 0.75
                ),  # 75% continuing employees
                "new_hire_active": int(
                    total_employees * 0.15
                ),  # 15% new hires who stayed
                "experienced_termination": int(
                    total_employees * 0.08
                ),  # 8% experienced terminations
                "new_hire_termination": int(
                    total_employees * 0.02
                ),  # 2% new hire terminations
            }
            ...
            if calculated_total != total_employees:
                ...
                status_breakdown["continuous_active"] += (
                    total_employees - calculated_total
                )
...
def run_optimization_loop_with_tracking(
    optimization_config, progress_container, tracker, visualizer
):
    """
    ...
    try:
        ...
        with progress_container:
            st.info(
                f"🎯 Starting {algorithm} optimization with target growth: {target_growth}%"
            )
            ...
            for iteration in range(max_iterations):
                ...
                progress_entry = OptimizationProgress(
                    ...
                    parameters={
                        ...
                        "merit_rate_avg": np.mean(
                            [
                                year_params.get("merit_base", {}).get(i, 0.03)
                                for i in range(1, 6)
                            ]
                        ),
                        "new_hire_adjustment": year_params.get(
                            "new_hire_salary_adjustment", {}
                        ).get(1, 1.1),
                    },
                )
                ...
                with st.spinner(
                    f"Running {sim_type} simulation iteration {iteration + 1}..."
                ):
                    simulation_success = run_simulation(
                        use_synthetic=not use_real_simulation
                    )
                ...
                if len(results["growth_rates"]) > 0:
                    current_growth = results["growth_rates"][
                        -1
                    ]  # Use final year, not average
                else:
                    current_growth = 0
                ...
                progress_entry.constraint_violations = {
                    "budget_constraint": max(
                        0, current_growth - 5.0
                    ),  # Don't exceed 5% growth
                    "feasibility_constraint": max(
                        0, -current_growth
                ...
                with convergence_chart.container():
                    conv_fig = visualizer.create_convergence_chart(
                        tracker.get_history(), target_value=0  # Target gap is 0
                    )
                ...
                with performance_metrics.container():
                    perf_data = visualizer.create_performance_dashboard(
                        tracker.get_history(), tracker.start_time
                    )
                    ...
                    with col3:
                        status = (
                            "✅ Converged" if abs(gap) <= tolerance else "🔄 Optimizing"
                        )
                ...
                if abs(gap) <= tolerance:
                    converged = True
                    st.success(
                        f"🎉 Optimization converged in {iteration + 1} iterations!"
                    )
                    break
        ...
        final_result = {
            ...
            "final_growth": iteration_results[-1]["current_growth"]
...
def run_scipy_optimization(optimization_config):
    ...
    try:
        ...
        st.info(
            "🔬 **Scientific Optimization**: Using scipy algorithms for precise convergence"
        )
        ...
        initial_merit_rates = year_params.get(
            "merit_base", {1: 0.03, 2: 0.03, 3: 0.03, 4: 0.02, 5: 0.02}
        )
        initial_merit_avg = np.mean(list(initial_merit_rates.values()))
        initial_hire_adj = year_params.get("new_hire_salary_adjustment", {}).get(
            1, 1.15
        )
        ...
        def objective_function(params):
            ...
            try:
                ...
                if iteration_count % 1 == 0:  # Show every iteration
                    ...
                    with col1:
                        st.metric(
                            f"Iteration {iteration_count}", f"{final_growth:.2f}%"
                        )
                    with col2:
        ...
        st.info(
            f"📊 Target: {target_growth}%, Initial: COLA={initial_cola:.1%}, Merit={initial_merit_avg:.1%}, Hire={initial_hire_adj:.0%}"
        )
        ...
        if result.success:
            ...
            st.success(
                f"🎯 Optimal parameters: COLA={optimal_cola:.1%}, Merit={optimal_merit:.1%}, Hire={optimal_hire:.0%}"
            )
            ...
            optimal_cola, optimal_merit, optimal_hire = result.x
            st.info(
                f"🎯 Best parameters found: COLA={optimal_cola:.1%}, Merit={optimal_merit:.1%}, Hire={optimal_hire:.0%}"
            )
...
def adjust_parameters_intelligent(gap, optimization_mode, iteration):
    ...
    try:
        ...
        gap_adjustment = (
            gap * adjustment_factor * 0.01
        )  # Convert percentage to decimal properly

        # Adjust COLA rate
        current_cola = year_params.get("cola_rate", {}).get(1, 0.03)
        new_cola = max(
            0.01, min(0.08, current_cola + gap_adjustment)
        )  # Bound between 1% and 8%
        ...
        for level in range(1, 6):
            ...
            new_merit = max(
                0.01, min(0.10, current_merit + (gap_adjustment * level_factor))
            )
        ...
        current_adj = year_params.get("new_hire_salary_adjustment", {}).get(1, 1.1)
        new_adj = max(
            1.0, min(1.4, current_adj + (gap_adjustment * 0.5))
        )  # Half the adjustment
        ...
        max_risk_level = getattr(
            st.session_state, "optimization_max_risk_level", RiskLevel.HIGH
        )
        if year_params:
            risk_engine = RiskAssessmentEngine()
            ...
            risk_assessment = risk_engine.calculate_overall_risk_assessment(
                parameter_changes=param_changes, implementation_timeline=90
            )
            ...
            if risk_levels_order.index(
                risk_assessment.overall_level
            ) > risk_levels_order.index(max_risk_level):
                st.warning(
                    f"⚠️ Iteration {iteration}: Parameter adjustment rejected due to {risk_assessment.overall_level.value} risk (max allowed: {max_risk_level.value})"
                )
                ...
                new_cola = max(
                    0.01,
                    min(0.08, current_cola + (gap_adjustment * conservative_factor)),
                )
                conservative_merit_rates = {}
                for level in range(1, 6):
                    current_merit = year_params.get("merit_base", {}).get(level, 0.03)
                    level_factor = (1.2 - (level * 0.1)) * conservative_factor
                    conservative_merit_rates[level] = max(
                        0.01, min(0.10, current_merit + (gap_adjustment * level_factor))
                    )
                new_adj = max(
                    1.0, min(1.4, current_adj + (gap_adjustment * 0.25))
                )  # Quarter adjustment
                ...
                if risk_levels_order.index(
                    conservative_risk.overall_level
                ) > risk_levels_order.index(max_risk_level):
                    st.error(
                        f"❌ Even conservative adjustment exceeds risk limit. Stopping optimization."
                    )
                    return False
                else:
                    st.success(
                        f"✅ Conservative adjustment accepted with {conservative_risk.overall_level.value} risk"
                    )
            else:
                st.info(
                    f"✅ Risk level {risk_assessment.overall_level.value} is within acceptable limits"
                )
        ...
        if update_parameters_file(new_params, target_years):
            ...
            st.info(
                f"   • Merit (avg): {np.mean(list(year_params.get('merit_base', {1:0.03}).values())):.1%} → {np.mean(list(new_merit_rates.values())):.1%}"
            )
    ...
    try:
        ...
        if use_synthetic:
            ...
            if proposed_params is None:
                proposed_params = {
                    ...
                    "new_hire_salary_adjustment": {
                        i: new_hire_adj for i in range(1, 6)
                    },
                }
            ...
            with st.spinner(
                "Running synthetic simulation... This will take 5-10 seconds."
            ):
                # Run synthetic simulation
                synthetic_results = run_synthetic_simulation(
                    proposed_params, target_years
                )
                ...
                if synthetic_results:
                    ...
                    st.info(
                        "💡 **Note**: These are synthetic results. Use 'Real Simulation' for final validation."
                    )

                    # Show quick summary
                    final_growth = (
                        synthetic_results["growth_rates"][-1]
                        if synthetic_results["growth_rates"]
                        else 0
                    )
                    final_salary = (
                        synthetic_results["avg_salaries"][-1]
                        if synthetic_results["avg_salaries"]
                        else 0
                    )
        ...
        if dbt_result.returncode != 0:
            ...
            if "Conflicting lock is held" in dbt_result.stdout:
                ...
                st.error(
                    "The simulation.duckdb file is locked by another process (likely your IDE)."
                )
                st.error(
                    "Please close any database connections in Windsurf/VS Code and try again."
                )
                st.info(
                    "💡 Tip: Look for open SQL tabs or database explorer connections."
                )
                return False
            else:
        ...
        dbt_result = subprocess.run(
            [
                ...
                "run",
                "--select",
                "stg_comp_levers int_effective_parameters",
            ],
            capture_output=True,
            text=True,
            cwd=str(dbt_dir),
        ...
        try:
            ...
            with st.spinner(
                "Running full multi-year simulation... This will take 3-5 minutes."
            ):
            ...
            if result.returncode == 0:
                    ...
                    if db_path.exists():
                        ...
                        try:
                            ...
                            if tables_exist >= 2:
                                ...
                                st.info(
                                    f"   • Workforce snapshot years: {[y[0] for y in snapshot_years]}"
                                )
                                st.info(
                                    f"   • Event years: {[y[0] for y in event_years]}"
                                )

                                if len(snapshot_years) == 1 and len(event_years) == 1:
                                    st.warning(
                                        "⚠️ Only single-year data found - simulation may not have processed all years correctly"
                                    )
                                elif len(snapshot_years) >= 5:
                                    st.success(
                                        "✅ Multi-year data found - full simulation successful!"
                                    )
                                else:
                                    st.success(
                                        f"✅ Multi-year data found - {len(snapshot_years)} years in database"
                                    )
                            else:
                                st.warning(
                                    "⚠️ Simulation tables not found - simulation may have failed"
                                )
                                st.info(
                            ...
                            st.warning(
                                f"⚠️ Could not verify simulation results: {table_error}"
                            )
                            st.info(
                                "The simulation may have failed to create the expected tables"
                            )
                        finally:
                            conn.close()
                    else:
                        st.warning(
                            "⚠️ Simulation database not found - simulation failed to create database"
                        )

                except Exception as e:
                    st.warning(f"Database verification error: {e}")
                    st.info(
                        "Unable to verify simulation results, but this doesn't necessarily mean the simulation failed"
                    )
                ...
                st.error(
                    f"❌ Dagster CLI method failed with return code: {result.returncode}"
                )
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "materialize",
                    "--select",
                    "simulation_year_state+",  # This should trigger multi-year
                    "-f",
                    "definitions.py",
                ],
                capture_output=True,
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    if db_path.exists():
                        ...
                        try:
                            ...
                            if tables_exist >= 2:
                                ...
                                if len(snapshot_years) >= 5:
                                    ...
                                    st.warning(
                                        f"⚠️ Only {len(snapshot_years)} year(s) found - trying fallback method"
                                    )
                            else:
                                st.warning(
                                    "⚠️ Simulation tables not found - trying fallback method"
                                )
                        ...
                        st.warning(
                            "⚠️ Simulation database not found - trying fallback method"
                        )
        ...
        try:
            ...
            with st.spinner(
                "Running multi-year simulation manually... This will take 3-5 minutes."
            ):
                for year in simulation_years:
                    ...
                    for model in models_to_run:
                        ...
                        if result.returncode != 0:
                            st.error(
                                f"Failed to run {model} for year {year}: {result.stderr}"
                            )
                            return False
...
with st.sidebar:
    ...
    if apply_mode == "Single Year":
        ...
        available_years = (
            sorted(current_params.keys())
            if current_params
            else [2025, 2026, 2027, 2028, 2029]
        )
        selected_year = st.selectbox(
            "Select Year", available_years, index=1 if len(available_years) > 1 else 0
        )
    else:
    ...
    # Simulation Settings
    st.markdown(
        '<div class="section-header">Simulation Settings</div>', unsafe_allow_html=True
    )
    ...
    if is_synthetic_mode:
        st.info(
            "🧪 **Synthetic Mode**: Uses mathematical models for instant results. Perfect for rapid parameter testing and optimization workflows."
        )
        st.caption(
            "⚡ Performance: ~5-10 seconds | 🎯 Accuracy: ~85-95% correlation with real simulation"
        )
    else:
        st.warning(
            "🔄 **Real Simulation Mode**: Runs complete dbt transformation pipeline. Use for final validation and production decisions."
        )
    ...
    # Parameter sections
    st.markdown(
        '<div class="section-header">COLA & Merit Rates</div>', unsafe_allow_html=True
    )
    ...
    cola_current = year_params.get("cola_rate", {}).get(
        1, 0.03
    ...
    st.markdown(
        '<div class="section-header">Promotion Parameters</div>', unsafe_allow_html=True
    )
    ...
    for level in range(1, 6):
        prob_current = year_params.get("promotion_probability", {}).get(
            level, default_promo_probs[level]
        )
        promo_probs[level] = (
            st.slider(
...
for param_name, level_values in proposed_params.items():
    for level, proposed_value in level_values.items():
        current_value = year_params.get(param_name, {}).get(
            level, 0.03
        )  # Default fallback
...
with tab1:
    st.markdown(
        '<div class="section-header">Parameter Overview</div>', unsafe_allow_html=True
    )
    ...
    with col2:
        ...
        st.metric(
            "Avg Merit Rate", f"{avg_merit:.1%}", f"{avg_merit - current_merit:+.1%}"
        )
        st.metric(
            "New Hire Adjustment",
            f"{new_hire_adj:.0%}",
            f"{new_hire_adj - current_adj:+.1%}",
        )

    # Parameter Sensitivity Quick View
    if year_params:
        st.markdown(
            '<div class="section-header">Parameter Sensitivity Quick View</div>',
            unsafe_allow_html=True,
        )
        st.info(
            "💡 Understanding which parameters have the most impact on your outcomes"
        )
    ...
    st.markdown(
        '<div class="section-header">Parameter Validation</div>', unsafe_allow_html=True
    )
    ...
    validation_result = validate_parameters_with_risk_analysis(
        proposed_params, year_params
    )
    ...
    if validation_result["warnings"]:
        for warning in validation_result["warnings"]:
            st.markdown(
                f'<div class="warning-box">⚠️ <strong>Warning:</strong> {warning}</div>',
                unsafe_allow_html=True,
            )
    ...
    if validation_result["risk_assessment"] and validation_result[
        "risk_assessment"
    ].overall_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
    ...
    if errors:
        for error in errors:
            st.markdown(
                f'<div class="error-box">❌ <strong>Legacy Error:</strong> {error}</div>',
                unsafe_allow_html=True,
            )
    ...
    if not errors and not warnings:
        st.markdown(
            '<div class="success-box">✅ <strong>All parameters validated successfully!</strong></div>',
            unsafe_allow_html=True,
        )
...
with tab2:
    st.markdown(
        '<div class="section-header">Impact Analysis & Sensitivity</div>',
        unsafe_allow_html=True,
    )
    st.info(
        "🔍 Preview the expected impact of your parameter changes and understand parameter sensitivity"
    )

    # Sub-tabs for Impact Analysis
    subtab1, subtab2, subtab3 = st.tabs(
        ["📊 Parameter Changes", "🎯 Sensitivity Analysis", "💼 Business Recommendations"]
    )
    ...
    with subtab1:
        ...
        if year_params:
            ...
            if abs(estimated_total_impact) < 0.1:
                st.markdown(
                    '<div class="warning-box">⚠️ <strong>Minimal Impact:</strong> Parameter changes are small and may not significantly affect outcomes.</div>',
                    unsafe_allow_html=True,
                )
            elif estimated_total_impact > 0:
                st.markdown(
                    '<div class="success-box">✅ <strong>Positive Impact:</strong> Parameter increases should improve compensation growth.</div>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    '<div class="error-box">📉 <strong>Negative Impact:</strong> Parameter decreases will reduce compensation growth.</div>',
                    unsafe_allow_html=True,
                )
            ...
            st.warning(
                "No baseline parameter data available. Unable to calculate impact analysis."
            )

    with subtab2:
        st.subheader("🎯 Parameter Sensitivity Analysis")
        st.info(
            "Understanding which parameters have the greatest impact on compensation outcomes"
        )
            ...
            **What is Sensitivity Analysis?**
            ...
            def calculate_sensitivity_scores():
                ...
                for level in range(1, 6):
                    ...
                    criticality = (
                        "High" if level <= 2 else "Medium" if level <= 4 else "Low"
                    )

                    # Calculate sensitivity score, handling zero base or delta
                    if merit_delta != 0 and level_merit != 0:
                        merit_sensitivity_score = abs(
                            weighted_impact / (merit_delta * 100)
                        )
                    else:
                ...
                for level in range(1, 5):  # Can't promote from level 5
                    level_promo_prob = year_params.get("promotion_probability", {}).get(
                        level, {1: 0.12, 2: 0.08, 3: 0.05, 4: 0.02}[level]
                    )
                    ...
                    promo_raise_amount = year_params.get("promotion_raise", {}).get(
                        level, 0.12
                    )
                    promo_impact = (
                        promo_delta * level_weights[level] * promo_raise_amount * 100
                    )
                    ...
                    sensitivity_data.append(
                        {
                            ...
                            "Sensitivity Score": abs(promo_impact / (promo_delta * 100))
                            if promo_delta > 0
                            else 0,
            ...
            sensitivity_df = sensitivity_df.sort_values(
                "Sensitivity Score", ascending=False
            )
            ...
            with col2:
                high_impact_params = sensitivity_df[
                    sensitivity_df["Business Criticality"] == "High"
                ]
                st.metric(
            ...
            with col3:
                high_budget_risk = sensitivity_df[
                    sensitivity_df["Budget Risk"].str.contains("High")
                ]
                st.metric(
                    "High Budget Risk",
            ...
            for criticality in ["High", "Medium", "Low"]:
                filtered_df = sensitivity_df[
                    sensitivity_df["Business Criticality"] == criticality
                ]
            ...
            display_df["Sensitivity Score"] = display_df["Sensitivity Score"].apply(
                lambda x: f"{x:.2f}"
            )
            ...
            for criticality in ["High", "Medium", "Low"]:
                with st.expander(
                    f"🎯 {criticality} Criticality Parameters",
                    expanded=(criticality == "High"),
                ):
                    crit_df = display_df[
                        display_df["Business Criticality"] == criticality
                    ]
            ...
            st.warning(
                "No parameter data available for sensitivity analysis. Please load baseline parameters first."
            )
    ...
    with subtab3:
        ...
        if year_params:
            ...
            second_param = (
                ...
                if "sensitivity_df" in locals() and len(sensitivity_df) > 1
                else None
            )
            ...
            if highest_param is not None:
                ...
                if "sensitivity_df" in locals():
                    high_risk_params = sensitivity_df[
                        sensitivity_df["Budget Risk"].str.contains("High")
                    ]
                ...
                # Low-impact warning
                if "sensitivity_df" in locals():
                    low_impact = sensitivity_df[
                        sensitivity_df["Sensitivity Score"] < 0.1
                    ]
            ...
            st.warning(
                "No parameter data available for recommendations. Please load baseline parameters first."
            )


with tab3:
    st.markdown(
        '<div class="section-header">Run Simulation</div>', unsafe_allow_html=True
    )
    ...
    if not errors:
        st.markdown(
            '<div class="success-box">✅ Parameters validated successfully</div>',
            unsafe_allow_html=True,
        )
        can_run = True
    else:
        st.markdown(
            '<div class="error-box">❌ Fix parameter errors before running</div>',
            unsafe_allow_html=True,
        )
        can_run = False
    ...
    if can_run:
        ...
        for level in range(1, 6):
            ...
            if abs(merit_rates[level] - current_merit_level) > 0.001:
                st.write(
                    f"• Merit Rate Level {level}: {current_merit_level:.1%} → {merit_rates[level]:.1%}"
                )
                changes_made = True
        ...
        with col2:
            simulation_button_text = (
                "🧪 Run Synthetic" if is_synthetic_mode else "🚀 Run Simulation"
            )
            ...
            if changes_made:
                with st.expander("⚠️ Pre-Simulation Risk Check (Click to view)"):
                    ...
                    for level in range(1, 6):
                        current_merit = (
                            year_params.get("merit_base", {}).get(level, 0.03)
                            if year_params
                            else 0.03
                        )
                        new_merit = merit_rates[level]
                        if (
                            abs(new_merit - current_merit) > 0.001
                        ):  # Only assess if there's a meaningful change
                            parameter_changes[("merit_base", level)] = (
                                current_merit,
                                new_merit,
                            )
                    ...
                    # COLA rate changes
                    current_cola = (
                        year_params.get("cola_rate", {}).get(1, 0.025)
                        if year_params
                        else 0.025
                    )
                    if abs(cola_rate - current_cola) > 0.001:
                    ...
                    current_adj = (
                        year_params.get("new_hire_salary_adjustment", {}).get(1, 1.1)
                        if year_params
                        else 1.1
                    )
                    if abs(new_hire_adj - current_adj) > 0.01:
                        parameter_changes[("new_hire_salary_adjustment", 1)] = (
                            current_adj,
                            new_hire_adj,
                        )
                    ...
                    if parameter_changes:
                        ...
                        quick_assessment = (
                            risk_engine.calculate_overall_risk_assessment(
                        ...
                        if quick_assessment.overall_level == RiskLevel.CRITICAL:
                            st.error(
                                "🚨 **CRITICAL RISK DETECTED** - Consider reviewing parameters before simulation"
                            )
                        ...
                        if quick_assessment.recommendations:
                            ...
                            for rec in quick_assessment.recommendations[
                                :3
                            ]:  # Show top 3
                                st.markdown(f"• {rec}")
                    else:
                        st.info(
                            "✅ No significant parameter changes detected - minimal risk"
                        )
            ...
            if st.button(simulation_button_text, type="primary", disabled=not can_run):
                ...
                if changes_made or is_synthetic_mode:
                    ...
                    st.info(
                        f"🚀 Calling {'synthetic' if is_synthetic_mode else 'real'} simulation function..."
                    )
                    if run_simulation(
                        use_synthetic=is_synthetic_mode,
                        ...
                        st.session_state.last_simulation_time = datetime.now()
                        if (
                            not is_synthetic_mode
                        ):  # Only show balloons for real simulation
                            st.balloons()
                    else:
                        st.session_state.simulation_running = False
    ...
    if st.session_state.last_simulation_time:
        ...
        st.write(
            f"Completed: {st.session_state.last_simulation_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
...
with tab4:
    st.markdown(
        '<div class="section-header">Impact Analysis</div>', unsafe_allow_html=True
    )

with tab5:
    st.markdown(
        '<div class="section-header">Simulation Results</div>', unsafe_allow_html=True
    )
    ...
    with col3:
        st.info(
            "Click 'Refresh Data' to reload results, 'Check DB' to see current database state"
        )
    ...
    try:
        ...
        if db_path.exists():
            conn = duckdb.connect(str(db_path))
            all_statuses_result = conn.execute(
                "SELECT DISTINCT detailed_status_code FROM fct_workforce_snapshot ORDER BY detailed_status_code"
            ).fetchall()
    ...
    sim_results = load_simulation_results(
        ...
        else ["continuous_active", "new_hire_active"]
    )
    ...
    if sim_results:
        ...
        result_type = (
            "🧪 Synthetic Results" if is_synthetic else "🔄 Real Simulation Results"
        )
                ...
                **Best Practices**:
                ...
                with col1:
                    if st.button("🔄 Validate with Real Simulation"):
                        ...
                        st.session_state.simulation_results = None
                        st.info(
                            "💡 Please switch to 'Real Simulation' mode in the sidebar and re-run simulation for validation."
                        )
                ...
                with col2:
                    if st.button("📊 Compare with Historical"):
                        ...
                        st.info(
                            "💡 Historical comparison feature available in next release."
                        )
        else:
            st.success(
                f"**{result_type}** | Last updated: {sim_results['last_updated']}"
            )
                ...
                if len(sim_results["avg_salaries"]) > 1:
                    ...
                    for i in range(1, len(sim_results["avg_salaries"])):
                        if sim_results["avg_salaries"][i - 1] > 0:
                            growth = (
                                (
                                    sim_results["avg_salaries"][i]
                                    - sim_results["avg_salaries"][i - 1]
                                )
                                / sim_results["avg_salaries"][i - 1]
                                * 100
                            )
        ...
        with col2:
            gap = (
                sim_results["target_growth"] - avg_growth
            )  # Use average growth for gap calculation
        ...
        if len(sim_results["years"]) == 1:
            ...
            st.info(
                "📋 Showing single-year simulation result with projected multi-year growth"
            )
            ...
            for i, year in enumerate(projected_years):
                if year == base_year:
                    ...
                    salary = base_salary * (
                        (1 + sim_results["target_growth"] / 100) ** years_diff
                    )
                    headcount = int(
                        base_headcount
                        * ((1 + sim_results["target_growth"] / 100) ** years_diff)
            ...
            st.caption(
                "* Projected years based on target growth rate | ✅ = Simulation results | 📊 = Target projections"
            )
            ...
            for i, year in enumerate(sim_results["years"]):
                ...
                growth_rate = (
                    sim_results["growth_rates"][i]
                    if i < len(sim_results["growth_rates"])
                    else 0
                )
                ...
                if (
                    "status_breakdown" in sim_results
                    and year in sim_results["status_breakdown"]
                ):
                    year_breakdown = sim_results["status_breakdown"][year]
                    for status in sim_results["available_statuses"]:
                        row_data[
                            f"{status.title()}"
                        ] = f"{year_breakdown.get(status, 0):,}"
            ...
            # Status filter info
            filter_info = (
                ", ".join(sim_results["status_filter"])
                if "status_filter" in sim_results
                else "active"
            )
            st.caption(
                f"📊 Calculations based on: **{filter_info}** status codes | 'Filtered Total' shows count for selected statuses"
            )
        ...
        **Need Help?**
        ...
        with col1:
            if st.button("🚀 Go to Run Simulation", type="primary"):
                st.switch_page(
                    "pages/1_Compensation_Tuning.py"
                )  # This will redirect to tab3
        with col2:
            if st.button("🎯 Try Parameter Tuning"):
                st.info(
                    "💡 Use the sliders in the sidebar to adjust parameters and generate synthetic results!"
                )

with tab6:
    st.markdown(
        '<div class="section-header">🤖 Auto-Optimize Parameters</div>',
        unsafe_allow_html=True,
    )
    st.info(
        "🎯 Let the system automatically find optimal parameters to meet your target growth rate"
    )

    # Import the new optimization progress module
    try:
        from optimization_progress import (
            OptimizationLogFilter, OptimizationProgress,
            OptimizationVisualization, ProgressTracker,
            create_optimization_progress_interface)

        progress_module_available = True
    except ImportError:
        st.warning(
            "⚠️ Advanced progress tracking module not available. Using basic interface."
        )
    ...
    with st.expander("🔧 Advanced Optimization Settings"):
        ...
        if use_synthetic_opt:
            st.info(
                "🧪 **Recommended**: Synthetic mode enables rapid convergence (~10-50 seconds total)"
            )
        else:
            st.warning(
                "🔄 **High Accuracy**: Real simulation mode for maximum precision (~20-50 minutes total)"
            )
    ...
    if current_results:
        current_growth = (
            np.mean(current_results["growth_rates"])
            if current_results["growth_rates"]
            else 0
        )
        gap = target_growth - current_growth
        ...
        with col3:
            gap_status = (
                "✅ At Target"
                if abs(gap) <= tolerance
                else f"{'📈 Above' if gap < 0 else '📉 Below'} Target"
            )
        ...
        if abs(gap) <= tolerance:
            st.markdown(
                '<div class="success-box">✅ <strong>Already at target!</strong> Current parameters are achieving the desired growth rate.</div>',
                unsafe_allow_html=True,
            )
        elif gap > 0:
            st.markdown(
                f'<div class="warning-box">📈 <strong>Need to increase growth by {gap:.2f}%</strong> - Parameters will be adjusted upward.</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<div class="warning-box">📉 <strong>Need to decrease growth by {abs(gap):.2f}%</strong> - Parameters will be adjusted downward.</div>',
                unsafe_allow_html=True,
            )
    else:
        st.warning(
            "⚠️ No current simulation results found. Run a simulation first to establish baseline."
        )
        st.info(
            "💡 Go to the 'Run Simulation' tab to run your first simulation before optimizing."
        )
    ...
    if st.button(
        "🚀 Start Auto-Optimization", type="primary", disabled=not can_optimize
    ):
        st.markdown("---")
        ...
        if use_scientific:
            ...
            with st.spinner(
                f"Running scientific optimization... This may take {estimation_time}."
            ):
            ...
            if enable_progress_tracking and progress_module_available:
                ...
                with status_container:
                    st.info(
                        f"🎯 Starting {algorithm_choice} optimization with {'real simulation' if use_real_simulation else 'synthetic functions'}"
                    )
                    st.info(
                        f"⏱️ Estimated time: {'20-50 minutes' if use_real_simulation else '2-5 minutes'}"
                    )

                # Run optimization with enhanced tracking
                with st.spinner(
                    f"Running {optimization_mode.lower()} optimization... Progress shown below."
                ):
                ...
                estimation_time = (
                    "20-50 minutes" if use_real_simulation else "2-5 minutes"
                )
                with st.spinner(
                    f"Running automated optimization... This may take {estimation_time}."
                ):
        ...
        if optimization_result:
            ...
            with col1:
                st.metric(
                    "Converged", "✅ Yes" if optimization_result["converged"] else "❌ No"
                )
            with col2:
                st.metric(
                    "Iterations Used",
                    f"{optimization_result['iterations']}/{max_iterations}",
                )
            with col3:
            ...
            if optimization_result["converged"]:
                st.markdown(
                    '<div class="success-box">🎉 <strong>Optimization Successful!</strong> Target growth rate achieved within tolerance.</div>',
                    unsafe_allow_html=True,
                )
                st.info(
                    "📊 Check the 'Results' tab to see the final optimized simulation results."
                )
                st.info(
                    "🎯 The optimized parameters have been automatically saved and applied."
                )
            else:
                st.markdown(
                    '<div class="warning-box">⚠️ <strong>Optimization Did Not Converge</strong> within the maximum iterations.</div>',
                    unsafe_allow_html=True,
                )
                st.info(
                    "💡 Try increasing max iterations or adjusting tolerance, or choose a different optimization strategy."
                )
            ...
            if optimization_result["iteration_history"]:
                ...
                fig.add_hline(
                    ...
                    line_dash="dash",
                    line_color="green",
                    annotation_text="Target Growth",
                    annotation_position="top left",
                )

                # Add tolerance bands
                fig.add_hrect(
                    y0=target_growth - tolerance,
                    y1=target_growth + tolerance,
                    fillcolor="green",
                    opacity=0.1,
                    annotation_text="Tolerance Zone",
                )
                ...
                display_history["Current Growth"] = display_history[
                    "current_growth"
                ].apply(lambda x: f"{x:.2f}%")
                display_history["Gap to Target"] = display_history["gap"].apply(
                    lambda x: f"{x:+.2f}%"
                )
                display_history["Converged"] = display_history["converged"].apply(
                    lambda x: "✅ Yes" if x else "❌ No"
                )
                display_history = display_history[
                    ["iteration", "Current Growth", "Gap to Target", "Converged"]
                ]
