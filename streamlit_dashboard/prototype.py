...
with st.sidebar:
    ...
    termination_rate = st.slider("Termination Rate", 0.0, 30.0, 12.0, 0.5, format="%g%%")
...
def generate_sample_data():
    ...
    level_dist = pd.DataFrame({
        ...
        'Count': np.random.multinomial(headcount, [0.4, 0.3, 0.2, 0.08, 0.02], len(years)).flatten()
    })
...
with tab1:
    ...
    with col3:
        avg_turnover = workforce_df['Terminations'].sum() / workforce_df['Headcount'].sum() * 100
...
with tab2:
    ...
    metrics_df['Turnover %'] = (metrics_df['Terminations'] / metrics_df['Headcount']) * 100
    
    # Format for display
    display_df = metrics_df[['Year', 'Headcount', 'Growth %', 'Hires', 'Terminations', 'Turnover %', 'Promotions']]
...
with tab3:
    ...
    # Stacked bar chart
    fig = go.Figure()
    fig.add_trace(go.Bar(name='Base Salary', x=comp_df['Year'], y=comp_df['Base Salary']))
    ...
    with col1:
        ...
        st.metric("Avg Comp Growth", f"{(comp_df['Total'].iloc[-1]/comp_df['Total'].iloc[0])**(1/4)-1:.1%}")
