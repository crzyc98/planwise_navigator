...
from .optimization_schemas import RiskLevel, get_parameter_schema
...
class ParameterConverter:
    ...
    def advanced_optimization_to_unified(
        ...
        **kwargs,
        ...
        metadata = OptimizationMetadata(
            ...
            if algorithm == "SLSQP"
            else OptimizationEngine.SCIPY_DE,
    ...
    def compensation_tuning_to_unified(
        ...
        **kwargs,
        ...
        if (
            "new_hire_salary_adjustment" in params
            and params["new_hire_salary_adjustment"]
        ):
            flat_params["new_hire_salary_adjustment"] = list(
                params["new_hire_salary_adjustment"].values()
            )[0]
    ...
    def unified_to_advanced_optimization(run: OptimizationRun) -> Dict[str, Any]:
        ...
        return {
            ...
            "algorithm": run.metadata.optimization_engine.value.replace(
                "scipy_", ""
            ).upper(),
            "parameters": run.results.optimal_parameters
            or run.configuration.initial_parameters,
            "objectives": {
    ...
    def unified_to_compensation_tuning(run: OptimizationRun) -> Dict[str, Any]:
        ...
        if "new_hire_salary_adjustment" in params:
            for level in range(1, 6):
                result["new_hire_salary_adjustment"][level] = params[
                    "new_hire_salary_adjustment"
                ]
    ...
    def format_for_streamlit_metrics(run: OptimizationRun) -> Dict[str, Any]:
        ...
        return {
            "summary_metrics": {
                ...
                "Algorithm": metadata.optimization_engine.value.replace(
                    "_", " "
                ).title(),
                "Runtime": f"{metadata.runtime_seconds:.1f}s"
                if metadata.runtime_seconds
                else "N/A",
                "Converged": "✅ Yes" if metadata.converged else "❌ No",
                "Evaluations": str(metadata.function_evaluations)
                if metadata.function_evaluations
                else "N/A",
            },
            "objective_metrics": {
                "Objective Value": f"{results.objective_value:.6f}"
            ...
            "business_metrics": {
                "Cost Impact": results.estimated_cost_impact.get("value", "N/A")
                if results.estimated_cost_impact
                else "N/A",
                "Employees Affected": results.estimated_employee_impact.get(
                    "count", "N/A"
                )
                if results.estimated_employee_impact
                else "N/A",
                "Confidence": results.estimated_cost_impact.get("confidence", "N/A")
                if results.estimated_cost_impact
                else "N/A",
            },
        }
            ...
            data.append(
                {
                    ...
                    "Contribution %": f"{(weighted_value / run.results.objective_value * 100):.1f}%"
...
class SessionStateIntegration:
    ...
    @staticmethod
    def save_current_optimization(
        run: OptimizationRun, interface_type: str = "advanced"
    ):
...
class ValidationUtils:
    ...
    def validate_optimization_run(run: OptimizationRun) -> Dict[str, Any]:
        ...
        if "merit_rate_level_1" in params and "cola_rate" in params:
            total_compensation_increase = (
                params["merit_rate_level_1"] + params["cola_rate"]
            )
            if total_compensation_increase > 0.15:  # 15%
                business_warnings.append(
                    "Combined merit and COLA rates exceed 15% - may impact budget significantly"
                )

        # Check promotion probability consistency
        promo_probs = [
            params.get(f"promotion_probability_level_{i}", 0) for i in range(1, 6)
        ]
        if any(promo_probs[i] > promo_probs[i - 1] for i in range(1, len(promo_probs))):
            business_warnings.append(
                "Higher-level positions have higher promotion probabilities than lower levels - check for logical consistency"
            )
        ...
        if "new_hire_salary_adjustment" in params:
            if params["new_hire_salary_adjustment"] > 1.25:  # 25% premium
                business_warnings.append(
                    "New hire salary adjustment above 25% may indicate market competitiveness issues"
                )
    ...
    def assess_business_impact(run: OptimizationRun) -> Dict[str, Any]:
        ...
        total_merit_impact = sum(
            params.get(f"merit_rate_level_{i}", 0) for i in range(1, 6)
        )
        cola_impact = params.get("cola_rate", 0) * 5  # Applied to all levels

        estimated_cost_impact = {
            "value": (total_merit_impact + cola_impact)
            * 1000000,  # Mock $1M per percentage point
            "confidence": "Medium",
...
def convert_legacy_results(
    legacy_results: Dict[str, Any], interface_type: str
) -> OptimizationRun:
...
if __name__ == "__main__":
    ...
    test_objectives = {"cost": 0.4, "equity": 0.3, "targets": 0.3}
