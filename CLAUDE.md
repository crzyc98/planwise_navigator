...
### **Compiled Execution Engine (Feature 119 / #470 — opt-in)**

An alternative simulation execution engine that runs dbt-compiled SQL directly against DuckDB behind the same orchestrator seam. **Opt-in; the default engine remains `dbt`** (Gate-5 decision 2026-07-19: exact parity proven at every scale, but this architecture cannot beat the dbt engine's post-#465 speed — its value is as the exact-parity reference oracle for the native-kernel program, #469/#471).

```bash
planalign simulate 2025-2027 --engine compiled     # per-run opt-in
planalign parity 2025-2027 --config C --census X --seed N   # two-engine exact-parity check (isolated DBs)
```

```yaml
# config/simulation_config.yaml
optimization:
  execution_engine: dbt   # dbt (default) | compiled
```

Key properties: byte-exact row-multiset parity with the dbt engine (`EXCEPT ALL` verified, duplicates included); one transaction per invocation with rollback; run-isolated dbt workspaces under `var/compiled_execution/<run_id>/` (never touches shared `dbt/target/`); typed delegation only (seed, hazard builds, `--full-refresh`, unknown semantics go through in-process dbt — recorded per run); generic errors fail with node/statement context, never silently replayed. Gate evidence: `var/compiled_execution/gate_evidence/`.
...
- Python 3.11 (engine package inside `planalign_orchestrator`); SQL generated by dbt-core 1.8.8 / dbt-duckdb 1.8.1 (unchanged as dev/test tool) + dbt programmatic API (`dbt.cli.main.dbtRunner`, already exercised in the Feature 116 investigation), `dbt/target/manifest.json` + compiled artifacts, `duckdb` Python client via the existing `DatabaseConnectionManager`; Feature 116 harness (`scripts/perf_profile/`) for acceptance measuremen (119-compiled-execution)
- no new tables; engine choice recorded in `run_metadata` provenance; per-run compiled-plan artifacts live in `dbt/target/` as today (119-compiled-execution)
