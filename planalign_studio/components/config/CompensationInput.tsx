...
export function CompensationInput({ value, onCommit, hasError, step = 500, min = 0 }: {
  ...
  return (
    <input
      ...
      className={`w-36 shadow-sm sm:text-sm rounded-md p-1 border text-right focus:ring-fidelity-green focus:border-fidelity-green ${hasError ? 'border-danger-border' : 'border-border-strong'}`}
    />
  );
}
