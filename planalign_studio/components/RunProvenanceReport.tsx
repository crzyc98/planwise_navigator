...
function dispositionStyle(disposition: string): string {
  if (disposition === 'fully_verified') return 'border-success-border bg-success-surface text-success-ink';
  if (disposition === 'incomplete') return 'border-warning-border bg-warning-surface text-warning-ink';
...
function Section({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-border bg-surface-raised shadow-sm">
      <h2 className="border-b border-border px-5 py-4 font-semibold text-ink">{title}</h2>
...
export default function RunProvenanceReport() {
  ...
  if (error || !envelope) {
    return (
      <div className="flex h-96 flex-col items-center justify-center text-center">
        ...
        <h1 className="text-xl font-semibold text-ink">Unable to open provenance report</h1>
        <p className="mt-2 max-w-xl text-ink-muted">{error ?? 'The archived report is unavailable.'}</p>
        <button
          onClick={() => navigate(scenarioId ? `/simulate/${scenarioId}` : '/simulate')}
          className="mt-5 rounded-lg bg-fidelity-green px-4 py-2 font-medium text-ink-inverse"
        >
          Back to Run History
        </button>
      </div>
  ...
  return (
    <div className="space-y-6 pb-8">
      <header className="rounded-xl border border-border bg-surface-raised p-6 shadow-sm">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h1 className="mt-1 text-2xl font-bold text-ink">Audit Report</h1>
              <p className="mt-2 break-all font-mono text-xs text-ink-muted">{evidence.run.run_id}</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="flex items-center rounded-lg bg-fidelity-green px-4 py-2 text-sm font-medium text-ink-inverse hover:bg-fidelity-dark disabled:opacity-60"
            >
            ...
            <button
              ...
              disabled={download !== null}
              className="flex items-center rounded-lg border border-border-strong px-3 py-2 text-sm text-ink-muted hover:bg-surface-subtle disabled:opacity-60"
            >
        ...
        {downloadError && <p className="mt-3 text-sm text-danger-ink">{downloadError}</p>}
      </header>
        ...
        <Section title="Missing or Unavailable Evidence">
          <div className="space-y-3">
            {report.missing_evidence.map(finding => (
              <div key={`${finding.field_path}-${finding.code}`} className="rounded-lg border border-warning-border bg-warning-surface p-3">
                <div className="flex flex-wrap items-center gap-2">
                  <code className="text-xs font-semibold text-warning-ink">{finding.field_path}</code>
                  <span className="rounded bg-warning-surface px-2 py-0.5 text-xs font-medium text-warning-ink">{finding.code}</span>
                  {finding.required && <span className="text-xs font-medium text-danger-ink">Required</span>}
                </div>
                <p className="mt-1 text-sm text-warning-ink">{finding.reason}</p>
              </div>
            ))}
          </div>
        </Section>
      ...
      <Section title="Software, Source, and Configuration">
        ...
        <details className="mt-5 rounded-lg border border-border bg-surface-subtle">
          <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-ink">Effective configuration</summary>
          <pre className="max-h-96 overflow-auto border-t border-border p-4 text-xs text-ink-muted">
      ...
      <Section title="Input Fingerprints">
        ...
        ) : <p className="mb-5 text-sm text-ink-muted">Census fingerprint unavailable.</p>}
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-surface-subtle text-left text-xs uppercase text-ink-muted">
            ...
            <tbody>
              {evidence.seed_files.map(seed => (
                <tr key={seed.logical_name} className="border-t border-border">
              ...
              {evidence.seed_files.length === 0 && <tr><td colSpan={3} className="px-3 py-4 text-center text-ink-muted">No seed fingerprints available.</td></tr>}
            </tbody>
          </table>
        </div>
      </Section>
      ...
      <Section title="Event Counts by Year">
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-surface-subtle text-left text-xs uppercase text-ink-muted"><tr><th className="px-3 py-2">Year</th><th className="px-3 py-2">Event type</th><th className="px-3 py-2 text-right">Count</th></tr></thead>
            <tbody>
              {evidence.event_counts.map(item => <tr key={`${item.simulation_year}-${item.event_type}`} className="border-t border-border"><td className="px-3 py-2">{item.simulation_year}</td><td className="px-3 py-2">{item.event_type}</td><td className="px-3 py-2 text-right">{item.count.toLocaleString()}</td></tr>)}
              {evidence.event_counts.length === 0 && <tr><td colSpan={3} className="px-3 py-4 text-center text-ink-muted">No completed-year event counts available.</td></tr>}
            </tbody>
          </table>
        </div>
      </Section>
      ...
      <Section title={`Captured Validation Results — ${evidence.validation_disposition.replaceAll('_', ' ')}`}>
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-surface-subtle text-left text-xs uppercase text-ink-muted"><tr><th className="px-3 py-2">Year</th><th className="px-3 py-2">Check</th><th className="px-3 py-2">Severity</th><th className="px-3 py-2">Outcome</th><th className="px-3 py-2 text-right">Affected records</th></tr></thead>
            <tbody>
              {evidence.validation_results.map(item => <tr key={`${item.simulation_year}-${item.check_name}-${item.severity}`} className="border-t border-border"><td className="px-3 py-2">{item.simulation_year}</td><td className="px-3 py-2">{item.check_name}</td><td className="px-3 py-2">{item.severity}</td><td className="px-3 py-2">{item.passed ? <span className="font-medium text-success-ink">Pass</span> : <span className="font-medium text-danger-ink">Fail</span>}</td><td className="px-3 py-2 text-right">{display(item.affected_record_count)}</td></tr>)}
              {evidence.validation_results.length === 0 && <tr><td colSpan={5} className="px-3 py-4 text-center text-ink-muted">No captured validation results available.</td></tr>}
            </tbody>
          </table>
        </div>
      </Section>
      ...
      <Section title="Integrity Verification and Reviewer Sign-Off">
        <div className="flex items-start gap-3 rounded-lg bg-surface-subtle p-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium text-ink">{report.digest.algorithm} · {report.digest.canonicalization}</p>
            <code className="mt-2 block break-all text-xs text-ink-muted">{report.digest.value}</code>
          </div>
          <button
            onClick={() => {
            ...
            className="flex shrink-0 items-center rounded-lg border border-border-strong bg-surface-raised px-3 py-2 text-sm text-ink-muted hover:bg-surface-subtle"
          >
