...
WITH current_workforce AS (
    -- Get all active employees for the current year
    SELECT
        employee_id,
        employee_ssn,
        employee_deferral_rate as baseline_deferral_rate,
...
final_state AS (
    SELECT
        ...
        (e.had_escalation_this_year = 1) as had_escalation_this_year,
        ...
        CASE
            ...
            THEN DATE_DIFF('year', e.first_escalation_date, DATE('{{ simulation_year }}-12-31'))
            ELSE NULL
        END as years_since_first_escalation,
        CASE
            WHEN e.last_escalation_date IS NOT NULL
            THEN DATE_DIFF('day', e.last_escalation_date, DATE('{{ simulation_year }}-12-31'))
            ELSE NULL
