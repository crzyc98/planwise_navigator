...
-- Preserve eligibility events from the existing table (generated by orchestrator MVP)
existing_eligibility_events AS (
  ...
  {% if not is_incremental() %}
    -- On full refresh, we need to handle the case where {{ this }} doesn't exist yet
    -- For full refresh, eligibility events will be regenerated by orchestrator
    AND FALSE
  {% endif %}
),

-- Combined events union
all_events AS (
  SELECT
    employee_id,
    employee_ssn,
  ...
  {% endif %}
  {% if is_incremental() %}
    -- Only preserve existing eligibility events during incremental updates
    AND simulation_year = {{ simulation_year }}
  {% endif %}
),
...
all_events AS (
  ...
  SELECT
    ...
    event_probability,
    event_category
  FROM existing_eligibility_events
...
SELECT
  ...
  -- Priority: termination > hire > eligibility > promotion > merit_increase
  ROW_NUMBER() OVER (
