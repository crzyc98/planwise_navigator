...
enrollment_events AS (
  SELECT
    ...
    ee.effective_date,
    ee.event_details,
    -- **CRITICAL COMPENSATION FIX**: Apply bounds checking to enrollment event compensation
...
synthetic_enrollment_events AS (
  SELECT
    ...
    se.effective_date,
    se.event_details,
    -- **CRITICAL COMPENSATION FIX**: Use baseline workforce compensation with bounds checking
