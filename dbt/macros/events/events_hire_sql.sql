{% macro events_hire_sql(cohort_table, simulation_year) %}
  {#-
    Generate hiring events for E068A fused event generation

    This macro extracts hiring event logic from int_hiring_events.sql
    ...
    Returns: SQL for hiring events CTE
  #}
  ...
  new_hire_assignments AS (
    SELECT
      ...
      'SSN-' || LPAD(CAST(900000000 + ({{ simulation_year }} - {{ var('start_year', 2025) }}) * 100000 + hs.hire_sequence_num AS VARCHAR), 9, '0') AS employee_ssn,
  ...
  SELECT
    ...
    'New hire - Level ' || nha.level_id AS event_details,
