...
print("=== Testing Compensation Event Chains ===")

# Run the compensation chain validation test
result = conn.execute("""
...
raise_after_promotion_validation AS (
    ...
    FROM fct_yearly_events r
    LEFT JOIN fct_yearly_events p
...
print("Compensation Chain Validation Results:")
print("Test Type                | Total | Passed | Failed | Pass Rate")
print("-" * 65)
for row in result:
    print(f"{row[0]:<23} | {row[1]:>5} | {row[2]:>6} | {row[3]:>6} | {row[4]:>7}%")

# Check if all tests passed (should be 100% pass rate)
all_passed = all(row[4] == 100.0 for row in result)
