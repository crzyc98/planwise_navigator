...
class TerminationEventGenerator(HazardBasedEventGeneratorMixin, EventGenerator):
    ...
    supports_sql = True
    supports_polars = True  # Has Polars implementation in polars_event_factory
    ...
    def generate_events(self, context: EventContext) -> List["SimulationEvent"]:
        ...
        if mode == "sql":
            logger.debug(
                f"termination: SQL mode - events generated by dbt models "
                f"{self.dbt_models} for year {context.simulation_year}"
            )
            return []

        # Polars mode is handled by polars_event_factory.py
        # This interface exists for consistency but returns empty in Polars mode too
        logger.debug(
            f"termination: Polars mode - events generated by polars_event_factory "
